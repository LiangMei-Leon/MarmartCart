...
- **Styles, modifiers, range sources and input behaviors are copyable with `StateCopy`**:
  `StateCopy.Copy` returns a detached copy of a `Style` or a modifier graph with the same configuration,
  and `StateCopy.TryReplay` updates a live copy from its source in place; `BaseModifier` implements
  `IStateIdentity`, so a replayed list pairs every modifier with its source node by `NodeId` even after
  the list was reordered.

### Changed

- **Breaking:** **`GlyphParamModifier.MarkParamsDirty` takes the changed parameter's `StateMember`**: a
  custom modifier deriving from `GlyphParamModifier<TParams>` passes the member its state callback
  received — a `[StateProperty(nameof(MarkParamsDirty))]` declaration needs no change — and
  `BaseModifier.MarkRenderDirty(bool)` accepts the member as an optional second argument.
- **Glyph atlas keeps laid-out text resident**: a glyph stays in the atlas for as long as a text laying
  it out is alive, whether or not that text is scrolled into view; the atlas shrinks when texts are
  disabled or destroyed, not while they scroll.
- **Preset copies live as long as the component**: a `UniText` makes its copy of each `StylePreset` and
  `ModifierGraphPreset` once, and a `UniTextEditable` its copy of each `InputBehaviorPreset`, instead of
  instantiating and destroying a hidden ScriptableObject copy on every enable; the copies keep the
  preset's later edits in place, including edits made while the component was disabled, and are
  replaced only when their authored node is removed, replaced or changes type.

### Removed

- **Breaking:** **`UniTextBuffers.hasValidGlyphCache`**: the field carried no information and nothing
  read it — delete any reference to it.

### Fixed

- **Draw-call spike while scrolling masked text**: scrolling a `UniText` under a `RectMask2D` pushed the
  frame's draw calls into the hundreds for a frame at a time and rasterized again the glyphs that had
  scrolled out of view.
- **Masked text re-meshed twice per scroll step**: each time the viewport moved past its margin, a
  `UniText` under a `RectMask2D` rebuilt its mesh in two consecutive frames instead of one.
- **Small texts under a `RectMask2D` re-meshed on every scroll step**: a text that fits inside the
  viewport rebuilt its mesh each time the viewport moved half its height, though nothing changed.
- **Toggling a `RectMask2D` disabled viewport culling for good**: after the mask was disabled and
  re-enabled in Play mode, texts under it kept their full meshes and no longer culled to the viewport.
- **Every text re-meshed whenever the glyph atlas compacted**: a compaction rebuilt the mesh of every
  `UniText` and `UniTextWorld` in the scene although their meshes were still valid.
- **Modifier from a preset stopped responding after disable and enable**: a modifier obtained with
  `GetModifier<T>()` from a `StylePreset` or a `ModifierGraphPreset` went dead once its `UniText` was
  disabled and enabled again, or when obtained while the component was disabled — setting its
  properties no longer updated the text; an `InputBehavior` from an `InputBehaviorPreset` obtained with
  `UniTextEditable.GetBehavior<T>()` went dead the same way.
- **`GetModifier<T>()` found nothing inside a `ModifierGraphPreset` before the first rebuild**: querying
  in `Start`, or while the component was disabled, returned `null` for a modifier the preset contains.
- **Editing an animation parameter on a `StylePreset` asset replaced every consumer's modifier**:
  changing, for example, the amplitude of a `WaveModifier` or a `DeformModifier` inside a preset, in the
  Inspector or through its properties at runtime, rebuilt that modifier on every `UniText` using the
  preset, so references obtained with `GetModifier<T>()` went dead; the value now updates in place.

## [3.5.11] - 2026-09-06

### Added

- **`reverse` parameter on `PaintOrderModifier`**: stacks a glyph-major range front to back, so the
  first glyph's stroke and shadow overlap the glyphs after it; cursively connected stretches such as
  Arabic words still compose as one unit and reverse per word, and layer order ignores the parameter.

### Changed

- **Touch editing follows the pointer, not the platform**: a touch press on a `UniTextEditable` opens the
  touch interaction — tap gestures, long-press selection, selection and insertion handles, the magnifier —
  and a mouse or pen press runs the desktop click, drag and right-click interaction, on every platform, so
  a touch laptop gets touch gestures on its screen and mouse behaviour from its trackpad, a phone or
  tablet with a mouse or pen gets desktop behaviour from it, and a touchscreen alone no longer decides.
- **LightSide Core 3.5.9 required**: the package now depends on `media.lightside.core` 3.5.9, up
  from 3.5.8.

### Fixed

- **Mouse clicks handled as touch in the Editor**: on a Windows machine that reports a touchscreen, real
  or virtual, clicking a `UniTextEditable` in Play mode showed the touch selection handles and right-click
  did nothing.
- **Right-click ignored on iOS, Android and mobile browsers**: a mouse right-click on a `UniTextEditable`
  on a touch platform opened no context menu; it now opens the menu at the pointer as on desktop.
- **Mouse and controller ray treated as a finger under the XR Interaction Toolkit**: with its
  `XRUIInputModule` driving the EventSystem, a mouse or a controller ray could not select text by
  dragging and holding the button opened the context menu as a touch long-press does.
- **`InputUtils.EditorTouchSimulationActive` true without simulation**: it reported `true` on any machine
  with a touchscreen; with LightSide Core 3.5.9 it reports only the Device Simulator or the Input
  System's Simulate Touch Input From Mouse or Pen.

## [3.5.10] - 2026-09-05

### Changed

- **Scene View text editing starts on Alt+double-click**: a plain double-click on a UniText in the Scene
  View no longer enters edit mode — hold Alt while double-clicking, or select the text and press F2.
- **`LightSideGammaToLinearIfNeeded` decodes vertex color in world passes**: in a world pass of a custom
  shader (`LightSide_Custom-URP.hlsl`, or `LightSide_Custom.cginc` under `LIGHTSIDE_WORLD`) the helper
  now converts the authored sRGB color to linear in a Linear project and `LightSideVert` applies it, so
  remove any such conversion you added to a world effect as a workaround.

### Fixed

- **World text too bright in Linear color space**: `UniTextWorld` drew every color — face, outline,
  shadow, gradient tint — noticeably lighter and paler than the same color on a Canvas `UniText` in a
  project rendering in Linear; world text now matches Canvas text.

## [3.5.9] - 2026-09-05

### Added

- **`Incremental` and `Yoyo` repeat modes for `UniTextDriver`**: the Repeat setting now offers every
  `MotionCycle` mode:
  - `Incremental` replays the timeline forward with each clip's `From`→`To` travel added once more per
    pass, so a driven value such as an animation modifier's `Phase` keeps advancing with no seam at the
    loop.
  - `Yoyo` runs the timeline back the way it came like `PingPong`, but reads each clip's easing from its
    own beginning.
- **`TextProcessSettings.IsUnbounded`**: tells whether a layout width or height means "no limit"
  (`FloatMax` or beyond) — the test to use instead of comparing against zero, which is a limit like any
  other.

### Changed

- **Breaking:** **Rule weight is no longer clamped to 0–1**: `RangeRuleInstance.Weight`, `SetWeight` and
  the `Playback` envelope pass any value through, so a transition's `Back` or `Elastic` easing overshoots,
  continuous parameters extrapolate past their endpoints, discrete ones hold the endpoint the weight lies
  beyond, and a caller that relied on saturation clamps before setting.
- **Breaking:** **`ParameterDescriptor.Lerp` extrapolates**: a position outside 0–1 no longer saturates —
  continuous value types extrapolate and discrete ones select the endpoint the position lies beyond, so
  clamp the position yourself where saturation was intended.
- **`IModifierRuleWeightReceiver` receives a saturated weight**: `SetRuleWeight` is guaranteed 0–1, so
  receivers no longer need a clamp of their own.
- **Padding taller than the rect keeps the top edge**: when top and bottom padding together exceed the
  rect height, the text now anchors below the top padding instead of above the bottom one, mirroring
  the left edge kept when padding exceeds the width.

### Fixed

- **`Back` and `Elastic` easing flattened at the endpoint**: a `UniTextDriver` clip or a state transition
  using an overshooting curve never overshot — the value stopped flat at its endpoint instead.
- **Alignment ignored in a zero-size rect**: a `RectTransform` with zero width or height drew the text
  hanging from the pivot's top-left corner whatever the alignment, while any positive size, however
  small, aligned as set — a zero-size rect now places the text at the pivot by its alignment, so a
  centred world label sits centred on its point instead of below and to the right of it.
- **Zero-size rect treated as having no limit**: with zero width or height, `preferredHeight` reported
  zero to layout groups and Content Size Fitter, Auto Size kept the previous frame's font size, and
  `EllipsisModifier` never truncated — a zero-size rect now measures, fits and truncates exactly like a
  one-pixel one.
- **Text drifting away from the pivot at negative rect sizes**: a `RectTransform` with negative width or
  height pushed the text further right and up the further the size went below zero — a negative size
  now behaves exactly as zero, at the pivot.
- **Analyzer warnings on Unity 6.6**: the package's editor scripts raised six UAC0005 and UAC0007
  warnings (`AppDomain.GetAssemblies()`, `Assembly.Location`) on every script compile — on Unity 6.4 and
  newer the editor now takes its assembly list and paths from Unity, which also stays correct when the
  editor reloads code through an `AssemblyLoadContext`.
- **Font Mapping cards named after the atlas or material**: the TMP migration tool titled a font's
  card by the first object serialized in its asset file — usually the atlas texture or its material,
  which keep the name the font was created under — so a renamed font showed a name that no longer
  exists in the project; a card is now titled by the asset's file name, shows its path and selects
  the asset with `Open`.
- **Wrong source font picked for a TMP font**: the migration tool guessed a font's TTF/OTF by family
  name, so a UniText font could be built from a sibling file — another weight, or Noto Serif SC for
  Noto Serif; the file the TMP font asset itself records is used now, a match on the asset's file
  name only where it records none, and a family name never.
- **Font Mapping never refreshing what a re-scan found**: a font's family, path and source file stayed
  as the first scan recorded them however the files were renamed or moved since; every scan now
  re-reads them, keeping only a source file chosen through `Browse…`.
- **Deleted TMP fonts lingering in Font Mapping**: a font asset removed from the project stayed listed
  as unmapped, holding back the components stage and letting `Finish every mapping the tool can`
  build a UniText font for it; an unmapped row whose TMP asset is gone now leaves the list at the
  next scan.

### Removed

- **Breaking:** **`DriverLoop`**: the obsolete enum is gone; the driver's repeat has been the inspector's
  Cycles count, `-1` until stopped, and `Repeat` (`MotionCycle`) since 3.3.2, with scenes and prefabs
  migrated then, so drop any remaining reference to the enum.

## [3.5.8] - 2026-09-05

### Added

- **IME control in `InputUtils`**, working on either input backend:
  - `SetImeEnabled` turns the operating system's IME composition on or off.
  - `SetImeCursorPosition` places the IME candidate window.
  - `CompositionString` reads the composition in progress.
- **`InputUtils.EditorTouchSimulationActive`**: whether the editor is currently simulating touch input,
  on either input backend.
- **`EventSystemBootstrap.AddInputModule`**: adds the input module matching the project's active input
  handling to a GameObject.

### Fixed

- **Builds rejecting `LightSide/UI`, `LightSide/World`, `LightSide/World Lit` and custom effects**:
  projects with GPU instancing or single-pass instanced XR failed to compile them with
  "Non system-generated input signature parameter () cannot appear after a system generated value".
- **`InvalidOperationException` thrown every frame once a text field took focus**: projects with Active
  Input Handling set to Input System Package only hit it as soon as an editable text was selected.
- **Touch text editing never activating in the editor under the Input System**: with Active Input
  Handling set to Input System Package only, the Device Simulator's touch was ignored and fields kept
  mouse behaviour.
- **Trapezoid, rhombus and regular-polygon shapes collapsed to zero size**: could render unpredictably
...
- **`SystemEmojiFont.GetDefaultEmojiFont` on macOS returns the system font's own path**: it previously
  returned the path of a temporary private copy of that file.

### Fixed

- **Per-text shader parameters missing from `MaterialModifier`**: assigning a material whose shader
  declares `_LightSideInst*` slots drew no parameter controls in the inspector, so the values reaching
  each text could only be set as raw vectors from code.
- **Stray marks along the edges of glyphs drawn by `MaterialModifier`**: the padded band around each
  glyph showed fragments of unrelated glyphs, and raising `Quad Padding Override` grew the marks
  instead of clearing the clipping it exists to prevent — the atlas now reserves rim to match the
  padding, which stops growing past the signed-distance spread it can store, and colour glyphs are
  left unpadded.
- **Keyboard suggestion picks that replace the typed word ignored on Android 13+**: tapping a
  suggestion offering a different word than the one typed, and the double-space shortcut that turns
  two spaces into a period, silently changed nothing under newer keyboards such as current Gboard.
- **Multi-second stall in `UniText.OnEnable` on macOS after every script recompile**: the first text
  enabled after a domain reload paused the editor while the system emoji font was prepared, and every
  other system font macOS or Linux resolves for fallback paid the same pause on its first use in a
  session — fonts the operating system resolves are now memory-mapped in place instead of being
  copied to the temp folder.
- **`ColorFontCore.CreateFromPath` as slow on every launch as on the first**: the private copy a font
  file is loaded through was rebuilt on every domain reload or launch; it is now reused as long as
  the file's path, size and timestamp are unchanged, so reopening a font by path is instant.

## [3.5.6] - 2026-09-03

### Changed

- **Breaking:** **Inline media `Pivot` anchors the content, not only the rotation**: a sprite that
  does not fill its `Size` box is held to the pivot instead of being centred, so the serialized
  default `(0,0)` pins it to the box's bottom-left — set `(0.5, 0.5)` on the entry or its keyed
  override to keep the previous centring.

### Fixed

- **Sprites imported with `Mesh Type: Tight` drawn stretched**: an inline sprite whose import trims
  the transparent border was drawn from the trimmed part of its texture across a box shaped for the
  whole sprite, so `Preserve Aspect` letterboxed it at the wrong ratio and its shadow, glow and
  stroke traced the same wrong outline.

## [3.5.5] - 2026-09-03

### Added

- **`DeformModifier`** (default tag name `deform`): bends the glyph outline per pixel instead of
  moving the glyph — it deforms the distance field the glyph is drawn from, so the edge keeps its
  anti-aliasing and the fill, stroke, shadow, glow and inner-shadow on that text bend along with the
  face. `Amplitude` and `Scale` are in glyph heights, so the result looks the same at every font
  size; `Drift` is how far the noise field travels per unit of `Phase`, and `Phase` is driven from
  outside like every other animation modifier — held still it renders a hand-drawn unevenness. It
  stacks with the vertex animations, which move the glyph as a rigid body. Amplitude stops growing
  the effect past the atlas's signed-distance spread, and colour glyphs are left undeformed.
- **`InlineMediaModifier.MemberStage`**: a custom inline-media modifier declares which rebuild stage
  each entry or override member needs, so a value it reads no earlier than mesh generation costs
  only a mesh rebuild.

### Fixed

- **Inline sprite `Size` and `Preserve Aspect` not taking effect**: editing either on a catalog entry
  or on a keyed override left the rendered height unchanged until an unrelated field such as
  `Bearing Offset` was edited.
- **Swapping a sprite on a catalog entry drew the wrong image**: the new sprite's texture was drawn
  with the previous sprite's region until the text was laid out again.
- **Inline object `Bearing Offset` applied twice**: entries placed by `ObjModifier` were shifted by
  double the offset authored on them — offsets tuned against that need halving.

## [3.5.3] - 2026-09-03

### Added

- **Every parameter explains itself** — modifiers, markup rules, interactive rules, actions,
  selectors, playbacks, inline-media entries and paint swatches now carry a tooltip on each field:
  what the value means, the unit it is read in, and what a special value such as `0`, `NaN` or
  `Inherit` does there. The parameter picker on a tag or per-range override list shows the same line
  under each entry and searches it, so a parameter can be found by what it does rather than by its
  name.
- **Style problems listed in the text inspector** — the Style section now shows the modifier-graph
  diagnostics the Style Preset and Modifier Graph Preset inspectors already showed: a rule with no
  playback, a parameter rule aimed at a node that is not in the graph, a parameter the target
  modifier does not declare, a value type or composition the parameter does not accept. Each names
  the path to the offending node, and a multi-selection labels them by object.

### Changed

- **An interactive rule is bound once it is completely configured**: while a document is being
  authored, a rule still missing its playback, target node, parameter or value stays unregistered
  and is reported in the inspector instead of throwing, so a rule can be filled in field by field.
  A graph wrong in shape rather than incomplete still throws, and outside authoring an incomplete
  rule fails as before.

### Fixed

- **Emoji and inline sprites stacked at the wrong depth**: a fill that leaves a colour glyph
  untouched (`Color Glyphs` = `Skip`) took no place in the layer order, so the glyph's own picture
  was drawn at the default fill position and every layer authored around that fill stacked against
  the picture in the wrong order. The picture now takes the position of the fill that leaves it
  alone — the topmost such fill when there are several — as a later fill's paint covers an earlier
  one on text, and the colour filters stacked above that position reach it too.
- **A `MaterialModifier` sub-mesh copied a glyph it cannot draw**: an emoji or inline sprite drawn
  from a texture outside the atlases was copied into the sub-mesh, where no text material can render
  it. Such glyphs are now left to whoever draws their picture.
- **Effects applied by an interactive rule stacked at the wrong depth**: the transient modifier of a
  `ModifierRule` never received a layer position, so a stroke, glow or fill applied on hover kept
  sequence 0 and collided with the bottom of the stack; a fill inside the rule also went uncounted
  when the text decided whether outward effects render behind it. The rule's modifier now stacks at
  the position of the `InteractiveModifier` that owns it.
- **An interactive rule's modifier missed the parallel pre-pass**: `PrepareForParallel` did not reach
  a `ModifierRule`'s modifier, so one that caches main-thread state there entered parallel layout
  without it.
...
- **`media.lightside.core` raised to 3.5.2**: nothing in UniText itself changed in this release —
  the version exists to carry the dependency's fix below.

### Fixed

- **No text on the Android Emulator**: every glyph-atlas upload was rejected as a stale target on a
  GL driver that reports an immutable texture but leaves the immutable-level count unanswered, so no
  glyph ever reached the atlas. An unanswered query is now read as no evidence rather than as a
  mismatch.
...
- **No text rendered under Proton and Wine**: a Windows player running on Steam Deck through Proton,
  or under Wine, threw `NotImplementedException` from the operating-system font lookup on every
  layout pass, so no `UniText` rendered and layouts collapsed even with a font assigned; text renders
  again and automatic OS font fallback resolves through the fonts Proton ships.
