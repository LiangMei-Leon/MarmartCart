...
namespace LightSide
{
    ...
    public class UniTextMeshGenerator
    {
        ...
        /// <summary>Atlas keys the emitted quads sample, each once — outline glyphs, colour tiles, silhouette fields — for the mesh's own refs; content residency is refd by the component from its request pass, never from here.</summary>
        ...
        internal const float VisibleWindowPadding = 0.5f;

        /// <summary>Local-Y band the last generation emitted completely; a side is infinite when nothing lay culled beyond it.</summary>
        private float emittedBandMin = float.NegativeInfinity;
        private float emittedBandMax = float.PositiveInfinity;
        private bool emittedUnwindowed = true;
        /// <summary>Local-Y extent of the paragraphs the last generation received; inverted when it received none.</summary>
        private float contentMin = float.PositiveInfinity;
        private float contentMax = float.NegativeInfinity;

        /// <summary>Whether the last generation no longer serves <paramref name="window"/>: the window reaches geometry that generation culled; the window is gone while the generation was windowed; or a window arrived over an unwindowed generation whose content runs past the padded window — the one shrink that pays, since a text entirely outside its window is culled by its renderer instead. Reads the completed generation only; never call while one runs.</summary>
        internal bool WindowRequiresRemesh(Rect? window)
        {
            if (window is not { } w) return !emittedUnwindowed;
            if (w.yMin < emittedBandMin || w.yMax > emittedBandMax) return true;
            if (!emittedUnwindowed) return false;
            var pad = w.height * VisibleWindowPadding;
            return contentMin <= w.yMax && contentMax >= w.yMin
                   && (contentMin < w.yMin - pad || contentMax > w.yMax + pad);
        }
        ...
        private bool currentFaceExternal;
        ...
        /// <summary>Whether the glyph currently in <see cref="onGlyph"/> draws from a texture outside the atlases, so its face quad carries no atlas pixels a text material could render.</summary>
        public bool HasExternalFace => currentFaceExternal;
        ...
        [ThreadStatic] private static HashSet<long> trackedTileKeys;
        ...
        /// <summary>Tracks a colour-tile or silhouette-field key once. Those keys never enter <see cref="glyphEntryCache"/>: a cached entry there answers <see cref="TryGetCachedGlyphEntry"/> for outline glyphs only.</summary>
        private static void TrackUsedKey(long key, ref PooledBuffer<long> keys)
        {
            if (trackedTileKeys.Add(key)) keys.Add(key);
        }
        ...
        /// <summary>
        /// Set when a layer other than a fill takes over rendering the current glyph's base face in its
        /// own sub-mesh — a <see cref="MaterialModifier"/> in <c>Replace</c> mode. The base-mesh face quad
        ...
        /// <summary>
        /// Set when a fill replaces the current glyph's face with a copy it paints itself — a texture fill's
        /// sub-mesh quad, a colour glyph's silhouette — so the face is not recorded. A later fill that
        /// leaves the glyph's own picture as its paint clears it, and the face is recorded again at that
        /// fill's position. Reset before each <see cref="onGlyph"/>.
        /// </summary>
        public bool fillReplacesFace;

        /// <summary>Whether something other than the base mesh draws the current glyph's face, so the face quad must not be recorded.</summary>
        public bool FaceTakenOver => baseFaceClaimed || fillReplacesFace;
        ...
        internal readonly List<GlyphBatchRasterizer.TierUpgradeRequest> tierUpgradeRequests = new();
        internal readonly List<GlyphBatchRasterizer.TileSizeUpgradeRequest> tileSizeUpgradeRequests = new();
        ...
        /// <summary>Gets the UV1 buffer: x = aspect (glyphW/glyphH) plus, above
        /// <see cref="DeformFieldTable.RowLaneStep"/>, a deform row + 1 (absent = aspect alone),
        /// y = faceDilate, z = per-glyph cluster index (monotonic per-line, transform-invariant),
        ...
        /// <summary>
        /// A colour glyph drawn from a texture outside the atlases whose face quad stayed unclaimed:
        /// the base mesh keeps the quad only as the anchor for effects, and whoever owns the glyph
        /// draws its picture at <see cref="sequence"/> with <see cref="blend"/> — the position the
        /// face would have held, a claiming fill's included.
        /// </summary>
        internal struct ExternalFace
        {
            public int faceBaseIdx;
            public int cluster;
            public int sequence;
        ...
        private PooledBuffer<ExternalFace> externalFaces;

        /// <summary>The unclaimed external-texture faces of this generation, in emission order; valid from main-pass finalization until the next generation.</summary>
        internal ReadOnlySpan<ExternalFace> ExternalFaces
            => externalFaces.data == null ? default : externalFaces.data.AsSpan(0, externalFaces.count);
        ...
        /// glyph's layers draw over every earlier glyph; a stretch marked
        /// <see cref="UniTextBuffers.PaintOrderReversed"/> mirrors its group numbers, so there earlier
        /// glyphs draw over later ones. Clusters outside the mode buffer join a trailing layer-major
        /// group, or form their own group above a trailing glyph-major or reversed one; quads in the
        /// overlay band (<see cref="overlayBandStart"/>) form the final group above every glyph. Runs
        /// split on blend changes always, and on sequence changes only
            ...
            ref PooledBuffer<SdfRun> runsBuffer, out int runCount)
        {
            ...
            {
                if ((modeData[c] & (byte)PaintOrder.Glyph) == 0) continue;
                anyGlyph = true;
                break;
            }
            ...
            {
                var mode = modeData[c];
                if (c > 0 && (mode != modeData[c - 1] || (mode & (byte)PaintOrder.Glyph) != 0))
                    group++;
                groupOf[c] = group;
                if ((mode & UniTextBuffers.PaintOrderReversed) != 0)
                {
                    if (reversedFrom < 0) reversedFrom = c;
                }
                else if (reversedFrom >= 0)
                {
                    MirrorGroups(groupOf, reversedFrom, c);
                    reversedFrom = -1;
                }
            }
            if (reversedFrom >= 0) MirrorGroups(groupOf, reversedFrom, modeCount);
            var trailingGroup = modeData[modeCount - 1] == (byte)PaintOrder.Layer ? group : group + 1;
            ...
            for (var i = 0; i < n; i++)
            {
                ...
                else if ((uint)cluster < (uint)modeCount)
                {
                    ...
                    glyphMajor = (modeData[cluster] & (byte)PaintOrder.Glyph) != 0;
                }
                else
                {
        ...
        /// <summary>Mirrors the group numbers of a reversed stretch onto their own interval, so its glyphs stack front to back while the stretch keeps its place among its neighbours.</summary>
        private static void MirrorGroups(int[] groupOf, int start, int end)
        {
            var sum = groupOf[start] + groupOf[end - 1];
            for (var c = start; c < end; c++) groupOf[c] = sum - groupOf[c];
        }
        ...
        /// the new tier in the same frame (<see cref="GlyphBatchRasterizer.ProcessUpgrades"/>). Grow-only.
        /// </summary>
        /// <remarks>
            ...
            UniTextFont.Core font, long varHash48, int[] ftCoords, float glyphH, float aspect)
        {
            ...
            tierUpgradeRequests.Add(new GlyphBatchRasterizer.TierUpgradeRequest
            {
                glyphKey = glyphKey,
        ...
        private void RequestFieldUpgradesIfNeeded(in ColorFaceField field)
        {
            ...
            if (currentTileSizeBoost > 0)
                tileSizeUpgradeRequests.Add(new GlyphBatchRasterizer.TileSizeUpgradeRequest
                {
                    glyphKey = field.key,
                    glyphIndex = field.glyphIndex,
        ...
        /// larger class and re-meshes its consumers the same frame (<see cref="GlyphBatchRasterizer.ProcessUpgrades"/>). The glyph's
            ...
            ReadOnlySpan<Paragraph> paragraphs)
        {
            ...
            trackedTileKeys ??= new HashSet<long>();
            trackedTileKeys.Clear();
            ...
            var culledBelow = false;
            var culledAbove = false;
            contentMin = float.PositiveInfinity;
            contentMax = float.NegativeInfinity;
            if (!paragraphs.IsEmpty)
            {
                contentMax = offY - paragraphs[0].topY;
                contentMin = offY - paragraphs[paragraphs.Length - 1].bottomY;
            }
            ...
            if (cullActive && CatZones.meshGenerator.Enabled)
                CatZones.meshGenerator.MeowFormat(
                    ...
                    debugName, visibleLocalYMin, visibleLocalYMax, offY, cullPad, paragraphs.Length, contentMin, contentMax);
                ...
                if (cullActive)
                {
                    if (i < glyphs.Length)
                    {
                        ...
                        if (cullParaIdx < paragraphs.Length)
                        {
                            ...
                            var paraLow = offY - para.bottomY - cullPad;
                            var paraHigh = offY - para.topY + cullPad;
                            if (!IsLocalBandVisible(paraLow, paraHigh))
                            {
                                if (paraHigh < visibleLocalYMin) culledBelow = true;
                                else culledAbove = true;
                    ...
                    else
                    {
                        ...
                        if (localY < visibleLocalYMin - cullPad) { culledBelow = true; continue; }
                        if (localY > visibleLocalYMax + cullPad) { culledAbove = true; continue; }
                    }
                }
            ...
            emittedUnwindowed = !cullActive;
            emittedBandMin = culledBelow ? visibleLocalYMin : float.NegativeInfinity;
            emittedBandMax = culledAbove ? visibleLocalYMax : float.PositiveInfinity;
        ...
        private void ApplyGlyphExtras(int count, PositionedGlyph[] allGlyphs, float offX, float offY)
        {
            ...
            for (var k = 0; k < count; k++)
            {
                ...
                if (!FaceTakenOver) AddSdfQuad(claimedFillSequence, baseIdx, claimedFillBlend);
        ...
        private void GenerateColorSegment(PooledList<int> glyphIndices, PositionedGlyph[] positionedGlyphs, UniTextFont.Core font)
        {
            ...
            for (var i = 0; i < glyphCount; i++)
            {
                ...
                var external = font.TryGetColorTexture(colorGlyphId, out _, out var uvMin, out var uvMax,
                    out var externalMetrics);
                if (external)
                {
                ...
                else
                {
                    ...
                    TrackUsedKey(colorKey, ref usedColorKeys);
                ...
                if (fieldRequests != null && (uint)cluster < (uint)fieldRequestCount && fieldRequests[cluster] != 0)
                {
                        ...
                        && fieldEntry.encodedTile >= 0 && fieldEntry.handle >= 0 && fieldEntry.metrics.height > 0f)
                    {
                        ...
                        TrackUsedKey(fieldKey, ref usedFieldKeys);
                ...
                currentFaceExternal = external;
                InvokeGlyphModifiersAndComplete(glyphIndex);
                if (!FaceTakenOver)
                {
                    if (external)
                        externalFaces.Add(new ExternalFace
                        {
                            faceBaseIdx = i0,
                            cluster = cluster,
                            sequence = claimedFillSequence,
                            blend = claimedFillBlend
                        });
                    else
                        AddSdfQuad(claimedFillSequence, i0, claimedFillBlend);
                }
