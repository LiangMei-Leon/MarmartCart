...
namespace LightSide
{
    ...
    public sealed class UniTextBuffers
    {
        ...
        /// <summary>Per-codepoint painter-order modes consumed by the mesh generator's quad sort: bit 0 holds the <see cref="PaintOrder"/> value, <see cref="PaintOrderReversed"/> marks a reversed range. Write via <see cref="PreparePaintOrders"/>; <c>count == 0</c> means the whole block is layer-major.</summary>
        internal PooledBuffer<byte> paintOrders;

        /// <summary>Bit of a <see cref="paintOrders"/> entry marking a codepoint of a reversed glyph-major range: the range's composition groups stack front to back while the range keeps its place among its neighbours.</summary>
        internal const byte PaintOrderReversed = 2;
