...
namespace LightSide
{
    ...
    public abstract partial class UniTextBase
    {
        ...
        /// <see cref="SetDirty"/> with the minimum-necessary flag. A preset-sourced instance is this
        /// component's own copy: it survives disable/enable and takes the preset's later edits in
        /// place; only replacing its authored node replaces the copy. A modifier inside a
        /// <see cref="ModifierGraphPreset"/> is reachable from the first query on, before the first
        /// rebuild.
        /// </remarks>
        ...
        /// <summary>Attaches the live styles first, so a graph that materializes on attachment (a <see cref="ModifierGraphPreset"/> copy) is visible to the walk from the first query on.</summary>
