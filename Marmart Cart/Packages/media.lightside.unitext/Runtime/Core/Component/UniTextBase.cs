...
namespace LightSide
{
...
#endif
    {
        ...
        /// <summary>Refs of the keys the displayed mesh samples; rewritten on every mesh apply.</summary>
        private GlyphAtlasRefs meshRefs;

        /// <summary>Refs of every key the shaped content requests from the atlases: staged by the request pass after the previous set is released, refd once the batch that rasterized them has run, held until the next request pass or teardown — residency follows the content, never the mesh window.</summary>
        private GlyphAtlasRefs contentRefs;
        ...
        public Rect? VisibleWindow
        {
            ...
            {
                visibleWindow = value;
                if (!isProcessing && meshGenerator != null && meshGenerator.WindowRequiresRemesh(value))
        ...
        /// by <c>(Left, Bottom)</c>, size shrunk by <c>(Left+Right, Bottom+Top)</c>. A negative rect
        /// size collapses to zero at the pivot; padding exceeding the size keeps the left and top
        /// edges. Main-thread only.
        /// </summary>
        public Rect GetPaddedRect() => ApplyPadding(rectTransform.rect);

        private Rect ApplyPadding(Rect outer)
        {
            var outerWidth = Mathf.Max(0f, outer.width);
            var outerHeight = Mathf.Max(0f, outer.height);
            var width = Mathf.Max(0f, outerWidth - padding.x - padding.z);
            var height = Mathf.Max(0f, outerHeight - padding.y - padding.w);
            var xMin = outerWidth > 0f ? outer.xMin : 0f;
            var yMax = outerHeight > 0f ? outer.yMax : 0f;
            return new Rect(xMin + padding.x, yMax - padding.w - height, width, height);
        }
        ...
        /// <summary>
        /// Tears down the attribute parser (when alive), then marks the component dirty
        /// (<see cref="UniTextDirty.Text"/>) so the next pipeline pass lazily rebuilds via
        /// <see cref="ValidateAndInitialize"/> → <see cref="EnsureAttributeParserCreated"/>; runtime
        /// preset copies survive and re-register with the new parser. The dirty flag is set
        /// unconditionally — a preset that was empty at init time has no parser yet, but may have
        /// just gained styles and now needs one created.
        /// </summary>
        ...
        private void OnAtlasEntriesCleared()
        {
            ...
            var meshChanged = meshRefs.DropMissing();
            var contentChanged = contentRefs.DropMissing();
            if (!meshChanged && !contentChanged) return;
            SetAppearanceDirty();
        }
        ...
        protected void DeInit(bool isDestroying = false)
        {
            CatZones.lifecycle.MeowFormat("[UniText] DeInit '{0}': isDestroying={1}, heldKeys={2}+{3}e",
                name, isDestroying, meshRefs.GlyphCount, meshRefs.ColorCount);
        ...
        private void UpdateGlyphAtlasRefCounts()
        {
            ...
            meshRefs.Update(RenderMode, ref meshGenerator.usedGlyphKeys, ref meshGenerator.usedColorKeys,
                ref meshGenerator.usedFieldKeys);

            CatZones.glyphAtlas.MeowFormat("[UniText] UpdateRefCounts '{0}': glyph={1}, color={2}, field={3}",
                name, meshRefs.GlyphCount, meshRefs.ColorCount, meshRefs.FieldCount);
        }
