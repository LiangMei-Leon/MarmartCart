    ...
    public abstract partial class UniTextBase
    {
        private sealed class StyleRuntimeSet : IStyleChangeSink
        {
            private sealed class Binding
            {
                ...
                internal bool IsStale => Version != SourcePreset.Version;
            }
            ...
            private bool structuralChangeSuppressed;
            ...
            internal void DetachParser()
            {
                ...
                try
                {
                    ReconcileParser(Array.Empty<Style>());
                }
                finally
                {
            ...
            internal void Listen() => ReconcilePresets(setDirty: false);
            ...
            {
                var changed = ResyncStaleBindings();
            ...
            private void OnPresetChanged(StylePreset preset, in StylePresetChange change)
            {
                ...
                try
                {
                    for (var i = 0; i < bindings.Count; i++)
                    {
                        ...
                        matched = true;
                        if (!TryReplay(binding, in change)) replaced |= ResyncBinding(binding);
                        binding.Version = preset.Version;
                    }
                }
                finally
                {
            ...
            private bool ResyncStaleBindings()
            {
                var changed = false;
                for (var i = 0; i < bindings.Count; i++)
                    if (bindings[i].IsStale) changed |= ResyncBinding(bindings[i]);
                return changed;
            }

            /// <summary>
            /// Aligns a binding's runtime styles with the preset's current authored styles: a style whose
            /// authored source is still present replays that source in place, any other gets a fresh copy.
            /// Returns whether the live style set must be re-registered.
            /// </summary>
            private bool ResyncBinding(Binding binding)
            {
                ...
                var preset = binding.SourcePreset;
                ...
                var changed = previous.Length != sources.Length;
                var context = new StateCopyContext();
                var suppressed = suppressStructuralChanges;
                suppressStructuralChanges = true;
                structuralChangeSuppressed = false;
                try
                {
                    for (var i = 0; i < sources.Length; i++)
                    {
                        ...
                        if (previousIndex >= 0 && context.TryReplay(previous[previousIndex], source))
                        {
                            current[i] = previous[previousIndex];
                            changed |= previousIndex != i;
                            continue;
                        }
                        current[i] = context.Copy(source);
                        changed = true;
                    }
                }
                finally
                {
                    suppressStructuralChanges = suppressed;
                }
                changed |= structuralChangeSuppressed;
            ...
            private void ApplyRuntimeChange(in StyleChange change)
            {
                if (change.Structural)
                {
                    if (suppressStructuralChanges)
                    {
                        structuralChangeSuppressed = true;
                        return;
                    }
            ...
            {
                for (var i = bindings.Count - 1; i >= index; i--) ReleaseBindingAt(i);
            }

            private void ReleaseBindingAt(int index)
            {
                var styles = bindings[index].RuntimeStyles;
