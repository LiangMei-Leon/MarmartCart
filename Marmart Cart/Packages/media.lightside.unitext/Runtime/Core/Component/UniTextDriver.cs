...
namespace LightSide
{
    ...
    public sealed partial class UniTextDriver : MonoBehaviour
    {
        ...
        [NonSerialized] private int cycle;
        ...
        /// <see cref="MotionCycle.PingPong"/> or <see cref="MotionCycle.Yoyo"/> return reads as the
        /// mirrored position and never as a phase past the end. Setting it seeks that many seconds into
        /// the run, counted from its first pass, and renders that state; <see cref="Progress"/> is the
        /// same position normalized.
        /// </summary>
        ...
        /// <summary>Moves the playhead to a normalized 0..1 position of the first pass and renders that state.</summary>
        ...
        /// advances carries the pass and its leg, which a normalized round trip cannot.
        /// </summary>
        ...
        /// <summary>
        /// Folds the phase into the period its repeat covers — one length, or two when a return leg mirrors
        /// it — so it stays finite however long playback runs, and resolves the timeline position that phase
        /// evaluates to along with the index of the pass it falls in.
        /// </summary>
        private float Fold(float length, out int pass)
        {
            pass = 0;
            ...
            if (IsInfinite)
            {
                cycle += Mathf.FloorToInt(phase / period) * (mirrors ? 2 : 1);
                phase = Mathf.Repeat(phase, period);
            }
            else
            {
                cycle = 0;
                ...
                if (phase >= run)
                {
                    pass = Mathf.Max(1, cycles) - 1;
                    return mirrors && (pass & 1) == 1 ? 0f : length;
                }
            }

            var leg = Mathf.FloorToInt(phase / length);
            var local = Mathf.Repeat(phase, length);
            pass = cycle + leg;
            return mirrors && (leg & 1) == 1 ? length - local : local;
        }

        private void Apply()
        {
            var length = TimelineLength;
            var time = Fold(length, out var pass);
            evaluated = time;
            if (!bound) return;
            Publish(length <= 0f ? 1f : time / length);

            var returning = cycleMode == MotionCycle.Yoyo && (pass & 1) == 1;
            var carried = cycleMode == MotionCycle.Incremental ? pass : 0;
            for (var c = 0; c < drives.Count; c++)
            {
                ...
                for (var i = 0; i < count; i++)
                {
                    ...
                    drive.Apply(i, returning
                        ? 1f - easing.Evaluate(1f - member)
                        : easing.Evaluate(member) + carried);
                }
            }
