...
namespace LightSide
{
    public abstract partial class UniTextBase
    {
        ...
        protected virtual void PrepareForParallel()
        {
            ...
            {
                var pad = window.height * UniTextMeshGenerator.VisibleWindowPadding;
        ...
        private static void ScanLayers(BaseModifier modifier, ref bool hasFill, ref int behindCount)
        {
            ...
            if (modifier.LayerChildren is { } children)
        ...
        private static void StampLayer(BaseModifier modifier, bool hasFill, ref int frontSeq, ref int behindSeq)
        {
            ...
            if (modifier.LayerChildren is not { } children) return;
            for (var i = 0; i < children.Count; i++)
            {
                if (children[i] != null)
        ...
        /// <summary>Opens a collection window and has each dirty component submit its content's glyph requests; the rasterizer owns everything from there.</summary>
        ...
        private void CollectGlyphRequests()
        {
                ...
                GlyphBatchRasterizer.Collect(tp.buf, fontProvider, RenderMode, ref meshRefs, ref contentRefs);
        }
        ...
        private static void MarkCollectedGlyphRequests(PooledBuffer<UniTextBase> components, int count)
        {
            for (int i = 0; i < count; i++)
            {
                ...
                if (tp == null || !tp.HasValidFirstPassData || tp.FontProviderForAtlas == null) continue;
                tp.HasValidGlyphsInAtlas = true;
                component.contentRefs.Commit(component.RenderMode);
            }
        }
        ...
        private static bool PostProcess()
        {
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                if (component.processingException != null || component.meshGenerator == null) continue;
                GlyphBatchRasterizer.QueueUpgrades(component.meshGenerator.tierUpgradeRequests,
                    component.meshGenerator.tileSizeUpgradeRequests);
            }
            UniTextDebug.BeginSample("AtlasUpgrades+Flush");
            GlyphBatchRasterizer.ProcessUpgrades();
            ...
            DeformFieldTable.Instance.Flush();
        ...
        private static void CompleteProcessing()
        {
            ...
            for (var i = 0; i < processingCount; i++)
            {
                ...
                if (component.meshGenerator != null && component.meshGenerator.WindowRequiresRemesh(component.visibleWindow))
                {
                    deferred |= UniTextDirty.Mesh;
                    deferredChanges |= DefaultCommitChanges(UniTextDirty.Mesh);
                }
