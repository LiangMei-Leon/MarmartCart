...
namespace LightSide
{
        ...
        IPointerEnterHandler, IPointerExitHandler, IPointerMoveHandler
    {
        ...
        /// <summary>
        /// Set by a gesture layer that runs its own recognition over this text's touch pointers
        /// (<see cref="PointerPressed"/> / <see cref="PointerReleased"/> — the editable's
        /// <see cref="TouchGestureRecognizer"/> session): the built-in long-press promotion stands
        /// down for touch holds so one hold never raises two competing context-menu requests, while
        /// the hold is still tracked so its release does not resolve into a click. Pen holds and
        /// right-click <see cref="ContextRequested"/> are unaffected.
        /// </summary>
        ...
        /// promotion of touch holds, except when a pointer subscriber consumed the original press and
        /// therefore owns its independent progress/context lifecycle. The press
        ...
        private void TickLongPress()
        {
            ...
            for (var i = 0; i < pointerSessionScratch.Count; i++)
            {
                ...
                var claimed = longPressClaimed && session.kind == PointerKind.Touch &&
        ...
        /// ids, so the legacy id-sign convention would misclassify it); a tracked XR pointer counts
        /// as a mouse — a precise, hover-capable ray. XR Interaction Toolkit module: its tracked-device
        /// event is that same ray, its pointer 0 is the mouse / pen, and every other id is a touch.
        /// Legacy input: negative pointerId = mouse buttons, non-negative = touch.
        /// </summary>
...
#endif
#if UNITEXT_XRI
            if (eventData is UnityEngine.XR.Interaction.Toolkit.UI.TrackedDeviceEventData)
                return PointerKind.Mouse;
            if (eventData.currentInputModule is UnityEngine.XR.Interaction.Toolkit.UI.UIInputModule)
