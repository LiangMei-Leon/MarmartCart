...
namespace LightSide
{
    ...
    public partial class RevealModifier : BaseModifier
    {
        ...
        [Tooltip("Name of the appearance effect used by ranges whose tag names none; empty selects the unnamed entry.")]
        ...
        [Tooltip("How much of the covered text is revealed, as a percentage or an absolute number of units; a fraction blends the unit at the frontier.")]
        ...
        [Tooltip("What one step of the frontier counts. Changing it converts an absolute frontier so the revealed text stays put.")]
        ...
        /// a re-parse where the other moves on a mesh rebuild.
        /// </summary>
        [Tooltip("Whether hidden text leaves the layout so the rest reflows, instead of keeping its space and just not being drawn.")]
        ...
        /// </remarks>
        [SerializeField, Parameter, StateProperty(nameof(MarkTextDirty))]
        [Tooltip("Label of a range the last shown line keeps room for, so it lands beside the frontier instead of below. Counts only when collapsing by lines.")]
        ...
        /// </summary>
        [Tooltip("Whether every range fills independently and at the same time, instead of one after another in text order.")]
        ...
        {
            if (TextProcessSettings.IsUnbounded(maxWidth)) return;
