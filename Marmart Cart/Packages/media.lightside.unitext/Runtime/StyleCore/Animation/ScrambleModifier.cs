...
namespace LightSide
{
    ...
    public partial class ScrambleModifier : BaseModifier
    {
        ...
        [Tooltip("Fraction of the text already decoded, from 0 fully scrambled to 1 real text.")]
        [SerializeField, SlotlessParameter(Invalidate = nameof(InvalidateParams)),
         Range(0f, 1f), StateProperty(nameof(ApplyProgressChange))]
        private float progress = 1f;

        /// <summary>Random re-picks per phase unit.</summary>
        [Tooltip("Random re-picks per phase unit.")]
        ...
        [Tooltip("Characters drawn at random in place of ones not yet decoded. Characters beyond the basic Unicode plane cannot be used.")]
        ...
        [Tooltip("Animation time input churning the random picks; it never advances on its own. Drive it from a driver, tween or Animator.")]
        [SerializeField, SlotlessParameter, StateProperty(nameof(MarkParamsDirty))]
        private float phase;

        private void MarkParamsDirty(StateMember member) => MarkRenderDirty(HasRanges, member);

        private void InvalidateParams() => MarkRenderDirty(HasRanges);

        private void ApplyProgressChange(StateMember member, float previous, ref float current)
        {
            current = Mathf.Clamp01(current);
            if (previous != current) MarkParamsDirty(member);
        }
