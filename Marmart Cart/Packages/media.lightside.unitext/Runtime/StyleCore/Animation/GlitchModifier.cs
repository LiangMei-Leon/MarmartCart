...
namespace LightSide
{
    ...
    public partial class GlitchModifier : EffectModifier
    {
        ...
        [Tooltip("How much of the text glitches at any moment, 0 none to 1 constant chaos.")]
        ...
        [Tooltip("Peak distance of the magenta/cyan copies from the glyph, in pixels.")]
        ...
        [Tooltip("Peak horizontal jerk in pixels.")]
        [SerializeField, Parameter, StateProperty(nameof(MarkParamsDirty))]
        private float amplitude = 2f;

        /// <summary>Corruption re-rolls per phase unit.</summary>
        [Tooltip("Corruption re-rolls per phase unit.")]
        [SerializeField, Parameter, StateProperty(nameof(MarkParamsDirty))]
        ...
        [Tooltip("Animation time input, in abstract units; the effect never advances on its own. Drive it from a driver, tween or Animator.")]
        [SerializeField, SlotlessParameter, StateProperty(nameof(MarkParamsDirty))]
        private float phase;

        private void MarkParamsDirty(StateMember member) => MarkRenderDirty(paramSets.Count > 0, member);
