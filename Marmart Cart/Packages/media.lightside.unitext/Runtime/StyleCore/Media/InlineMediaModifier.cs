...
namespace LightSide
{
        ...
        where TEntry : InlineMedia
    {
        ...
        /// <summary>
        /// Cheapest rebuild stage that carries a change of <paramref name="member"/> — of a catalog
        /// entry or of a keyed override — to the screen. Size, pivot and rotation are values this
        /// class only forwards to <see cref="OnMediaShaped"/>, so they default to
        /// <see cref="UniTextDirty.Positions"/>: any value a subclass turns into shaping-slot or
        /// positioned-glyph output is stale under <see cref="UniTextDirty.Mesh"/>, which reuses the
        /// positioned glyphs. An override classifies the members its own entry and override types
        /// declare, and lowers a forwarded member to <see cref="UniTextDirty.Mesh"/> only when it
        /// consumes that member no earlier than mesh generation.
        /// </summary>
        /// <exception cref="ArgumentOutOfRangeException"><paramref name="member"/> belongs to no
        /// classified type.</exception>
        protected virtual UniTextDirty MemberStage(StateMember member)
        {
            if (member == InlineMedia.Members.Advance ||
                ...
                member == InlineMediaOverride.Members.Advance ||
                member == InlineMediaOverride.Members.LineHeightAbove ||
                member == InlineMediaOverride.Members.LineHeightBelow)
                return UniTextDirty.Layout;
            if (member == InlineMedia.Members.BearingOffset ||
                member == InlineMedia.Members.Size ||
                member == InlineMedia.Members.Pivot ||
                member == InlineMedia.Members.Rotation ||
                member == InlineMediaOverride.Members.BearingOffset ||
                member == InlineMediaOverride.Members.Size ||
            ...
            if (member == InlineMedia.Members.Name ||
                member == InlineMediaOverride.Members.Key)
                return UniTextDirty.Text;
            throw new ArgumentOutOfRangeException(nameof(member));
        }

        private void ApplyEntryChange(IStateMemberReplay source, StateMember member)
            => ChangeSink?.MarkModifierChanged(this, MemberStage(member), source, member);
        ...
        where TOverride : InlineMediaOverride
    {
        ...
        private void OnOverrideChanged(IStateChangeSource source, in StateChange change)
        {
            ...
            if (member == InlineMediaOverride.Members.Key)
            {
                ...
                ChangeSink?.MarkModifierChanged(this, MemberStage(member),
        ...
        where TOverride : InlineMediaOverride
    {
        ...
        /// <summary>
        /// Places each visible occurrence's wrapper over its positioned glyph, whose coordinates
        /// already carry the media's bearing offset.
        /// </summary>
        private void OnRebuildEnd()
        {
            ...
            for (var i = 0; i < glyphs.Length; i++)
            {
                ...
                var glyph = glyphs[i];
                CreateInstance(in media, cluster, glyph.x, -glyph.y,
