...
namespace LightSide
{
    ...
    public sealed class SpriteFont : ColorFontCore
    {
        ...
        /// sprite's whole rect — the same rectangle <c>aspect</c> measures, so the picture fills the
        /// nominal box exactly: a <see cref="SpriteMeshType.Tight"/> sprite's mesh and
        /// <see cref="Sprite.textureRect"/> cover only its opaque pixels, and
        /// <see cref="Sprite.textureRectOffset"/> puts the trimmed rect back where it was authored. A
        /// tightly packed sprite has no readable <see cref="Sprite.textureRect"/>, so its region is
        /// extrapolated from its mesh instead, and the mesh is kept so the region's neighbours never show.
        /// </summary>
        private static SpriteGlyph Capture(Sprite sprite)
        {
            ...
            {
                var pixelMin = sprite.textureRect.min - sprite.textureRectOffset;
                var regionMin = new Vector2(pixelMin.x / texture.width, pixelMin.y / texture.height);
                var regionSpan = new Vector2(rect.width / texture.width, rect.height / texture.height);
                return new SpriteGlyph(sprite, texture, regionMin, regionMin + regionSpan, aspect, null, null, null);
            }
