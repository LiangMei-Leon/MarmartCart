...
namespace LightSide
{
    ...
    public partial class InlineSprite : InlineMedia
    {
        ...
        [Tooltip("Keeps the sprite's native aspect ratio, letterboxed inside the size box; off stretches it to fill.")]
        [SerializeField, StateProperty]
    ...
    {
        [Tooltip("Where the sprite's colour comes from: its own entry, the text component's colour, or an explicit override.")]
        public SpriteColorSource source;
        [Tooltip("Colour used when Source is Override.")]
    ...
    public sealed partial class InlineSpriteOverride : InlineMediaOverride
    {
        ...
        [Tooltip("Colour of this sprite: Original keeps the entry's own, Inherit takes the text colour, Override sets one here.")]
        ...
        [Tooltip("Whether the sprite keeps its native aspect ratio; Inherit uses the entry's setting.")]
    ...
    public sealed partial class SpriteModifier : InlineMediaModifier<InlineSprite, InlineSpriteOverride>
    {
        ...
        /// <inheritdoc/>
        protected override UniTextDirty MemberStage(StateMember member)
        {
            if (member == InlineSprite.Members.Color ||
                member == InlineSpriteOverride.Members.Color ||
                member == InlineMedia.Members.Rotation ||
                member == InlineMediaOverride.Members.Rotation)
                return UniTextDirty.Mesh;
            if (member == InlineSprite.Members.Sprite ||
                member == InlineSprite.Members.PreserveAspect ||
                member == InlineSpriteOverride.Members.PreserveAspect)
                return UniTextDirty.Positions;
            return base.MemberStage(member);
        }
        ...
        private void OnRebuildStart() => emitter.Clear();

        protected override void OnShapingStarted() => placements.Clear();

        /// <summary>
        /// Resolves the occurrence's glyph and box: the sprite keeps its aspect inside the box, held to the
        /// box's pivot, or stretches to fill it; the offsets land in the shaping slot, the scales in the placement. The
            ...
            float fontSize)
        {
            ...
            if (preserveAspect)
            {
                ...
                offsetX = (box.x - aspect * scale) * media.pivot.x;
                offsetY = (box.y - scale) * media.pivot.y;
            }
            else
            {
                scale = box.y;
        ...
        /// <summary>Applies the occurrence's tint, horizontal stretch and rotation to its face quad.</summary>
        ...
        /// <summary>
        /// Draws every occurrence whose face stayed unclaimed — a fill that took the face paints a
        /// silhouette in its place — by copying the finished face into its texture's sub-mesh at the layer
        /// position the face was given.
        /// </summary>
        private void OnMainPassFinalize()
        {
            if (placements.Count == 0) return;
            var gen = uniText.MeshGenerator;
            var faces = gen.ExternalFaces;
            for (var i = 0; i < faces.Length; i++)
            {
                ref readonly var face = ref faces[i];
                if (!placements.TryGetValue(face.cluster, out var placement)
                    || !font.TryGetSpriteGlyph(placement.glyphId, out var sprite)) continue;
                emitter.AppendTexturedFace(gen, face.faceBaseIdx, sprite.texture, face.sequence, face.blend,
