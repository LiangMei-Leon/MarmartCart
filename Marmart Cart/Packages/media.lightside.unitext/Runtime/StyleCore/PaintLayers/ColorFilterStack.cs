...
namespace LightSide
{
    ...
    internal sealed class ColorFilterStack
    {
        ...
        public void ApplyToFace(UniTextMeshGenerator gen)
        {
            ...
            ApplyToFace(gen, UniTextMeshGenerator.DefaultFillSequence);
        }

        /// <summary>
        /// Applies the filters stacked above <paramref name="sequence"/> to the current face quad, whoever
        /// claimed it — for a layer whose picture stands in for its paint at that position. A colour
        /// glyph's matrix row is rewritten, so a later call with a higher position replaces an earlier one.
        /// </summary>
        public void ApplyToFace(UniTextMeshGenerator gen, int sequence)
        {
            if (modifiers.Count == 0) return;

            var index = ResolveIndex(gen.currentCluster, sequence);
            ...
            if (gen.font.IsColor)
            {
                ...
                if (code == 0f && gen.Uvs3 == null) return;
