...
namespace LightSide
{
        ...
        IModifierRuleWeightReceiver
    {
        ...
        [Tooltip("How a gradient or texture spreads over the text: whole block, per line, per glyph, or per range. Inherit uses the swatch's value.")]
        ...
        [Tooltip("Gradient shape (linear, radial, angular); Inherit uses the swatch's value.")]
        ...
        [Tooltip("How a texture fits its frame: stretch, contain, cover, or tile; Inherit uses the swatch's value.")]
        [SerializeField, Parameter, Inheritable, StateProperty(nameof(MarkMeshDirty))]
        ...
        [Tooltip("Rotation of the gradient or texture, in degrees. NaN inherits from the swatch.")]
        ...
        [Tooltip("Zoom of the gradient or texture; 0 or less counts as 1. NaN inherits from the swatch.")]
        ...
        [Tooltip("Shift of the gradient or texture, in fractions of its frame. NaN per axis inherits from the swatch.")]
        ...
        [Tooltip("Corner shape of the outline or shadow edge: Round is always clean, Sharp keeps mitred corners up to Miter Limit. Single-channel SDF fonts draw Sharp as Round.")]
        [SerializeField, Parameter, StateProperty(nameof(MarkMeshDirty))]
        ...
        [Tooltip("Longest mitre a Sharp corner may grow, as a multiple of the edge width; spikier corners are clipped.")]
        ...
        [Tooltip("Colour multiplied with the solid, gradient, or texture paint.")]
        [SerializeField, Parameter, StateProperty(nameof(MarkMeshDirty))]
        ...
        protected override void OnGlyphEffect()
        {
            ...
            if (colorFace && (!sel.colorGlyphs || !gen.HasColorFaceField))
            {
                if (ClaimsBase)
                {
                    ClaimFillSlot(gen, in sel, out _, out _);
                    gen.fillReplacesFace = false;
                    gen.filters.ApplyToFace(gen, LayerSequence);
                }
                return;
            }
            ...
            if (claims)
            {
                ClaimFillSlot(gen, in sel, out var outputSequence, out var outputBlend);
                if (sel.paint.kind == PaintSourceKind.Texture)
                {
                    gen.fillReplacesFace = true;
        ...
        /// <summary>
        /// Takes the fill role for the current glyph: its face is ordered at this layer's position with
        /// the range's blend. A colour glyph the fill leaves untouched keeps its picture there, standing
        /// in for the paint, so the layers around it stack as they do around painted text; every later
        /// fill that leaves it untouched moves the picture up to its own position, as a later fill's
        /// paint would cover an earlier one on text.
        /// </summary>
        private void ClaimFillSlot(UniTextMeshGenerator gen, in LayerRange sel, out int sequence,
            out LayerBlend blend)
        {
            gen.fillClaimedThisGlyph = true;
            sequence = gen.hasClaimedFillLayerOverride
                ? gen.claimedFillSequenceOverride
                : LayerSequence + gen.sequenceBias;
            blend = gen.hasClaimedFillBlendOverride
                ? gen.claimedFillBlendOverride
                : sel.paint.blend;
            gen.claimedFillSequence = sequence;
            gen.claimedFillBlend = blend;
        }
