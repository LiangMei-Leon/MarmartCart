...
namespace LightSide
{
    ...
    /// overlaps earlier glyphs' fill; <c>reverse</c> stacks a glyph-major range front to back instead,
    /// so earlier glyphs overlap later ones. The axis is a single per-component channel — all layer modifiers
    ...
    public sealed partial class PaintOrderModifier : BaseModifier
    {
        ...
        /// <summary>Stacks glyph-major glyphs front to back — an earlier glyph's stroke or shadow overlaps later glyphs — when the markup carries no value. Layer order has no direction and ignores it.</summary>
        [SerializeField, Parameter, StateProperty(nameof(MarkMeshDirty))]
        [Tooltip("Glyph order stacks the first glyph on top instead of the last. Ignored by Layer order.")]
        ...
        /// their base's mode so a connected stretch stays one contiguous composition group. A reversed
        /// range carries <see cref="UniTextBuffers.PaintOrderReversed"/> on every stamp, connected
        /// stretches included, so the mesh generator mirrors the range as a whole.
        /// </summary>
        protected override void OnApply(in RangeApplyContext context)
        {
            ...
            if (resolved == PaintOrder.Layer)
            {
                for (var i = start; i < limit; i++)
                    Stamp(win, modes, i, packed, (byte)PaintOrder.Layer);
                return;
            }
            ...
            var reversedBit = Param.Reverse.Resolve(this, in context) ? UniTextBuffers.PaintOrderReversed : (byte)0;
            var stretchMode = (byte)((byte)PaintOrder.Layer | reversedBit);
            var glyphMode = (byte)((byte)PaintOrder.Glyph | reversedBit);

            var prevJoinsFollowing = PrecedingBaseJoinsFollowing(cp, start, provider);
            var baseMode = stretchMode;
            var hasBase = false;
            for (var i = start; i < limit; i++)
            {
                ...
                baseMode = connected ? stretchMode : glyphMode;
                hasBase = true;
        ...
        private static void Stamp(long[] win, Span<byte> modes, int i, long packed, byte mode)
        {
            var prev = win[i];
