...
namespace LightSide
{
    ...
    public sealed partial class FixedRangeSource : RangeSource
    {
        ...
        public struct Entry
        {
            [Tooltip("Codepoint range such as 0..5 or ^10.. where ^ counts from the end; empty covers the whole text.")]
            [SerializeField] private string range;
            [Tooltip("Parameter passed to the modifier for this range, written like a tag value.")]
        ...
        [Tooltip("Codepoint ranges the modifier applies to, each with an optional parameter.")]
