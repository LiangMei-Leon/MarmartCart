...
namespace LightSide
{
        ...
        public void Rebuild()
        {
            ...
            for (var i = 0; i < outputs.Count; i++)
            {
                ...
                for (var receiver = 0; receiver < weightReceivers.Count; receiver++)
                    weightReceivers[receiver].SetRuleWeight(Mathf.Clamp01(output.Weight));
    ...
    public sealed class RangeRuleInstance : IPlaybackHost
    {
        ...
        /// <summary>
        /// Blend weight of the contributions: the cascade at 0, the rule's target at 1, and extrapolation
        /// past either, which a transition's easing may reach in flight.
        /// </summary>
        public float Weight
        {
            get => playback.Weight;
        ...
        /// <summary>Sets the blend weight immediately, without choosing a tween implementation.</summary>
        public void SetWeight(float value)
        {
            if (!targetAlive) throw new ObjectDisposedException(nameof(RangeRuleInstance));
