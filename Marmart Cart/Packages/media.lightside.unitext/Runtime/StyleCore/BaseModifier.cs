...
namespace LightSide
{
    ...
    public abstract partial class BaseModifier : IStateIdentity
    {
        [SerializeField, HideInInspector, StatePassive]
        private ModifierNodeId modifierNodeId = ModifierNodeId.Create();

        bool IStateIdentity.SameStateIdentity(object other)
            => other is BaseModifier modifier && modifier.NodeId == NodeId;
        ...
        /// <summary>
        /// Whether this modifier belongs to a document being authored rather than to live player
        /// state. Latched from <see cref="StateAuthoring.IsActive"/> on every attachment and by
        /// <see cref="RefreshAuthoring"/>, so deferred work on any thread reads the value recorded
        /// at the change.
        /// </summary>
        private protected bool IsAuthoring => isAuthoring;

        /// <summary>Re-latches <see cref="IsAuthoring"/> from the change being applied.</summary>
        private protected void RefreshAuthoring() => isAuthoring = StateAuthoring.IsActive;
        ...
        /// <summary>
        /// Modifiers the layer stamp orders directly after this one: the aggregated
        /// <see cref="Children"/> by default; a modifier that carries transient graphs returns those,
        /// so their layers stack at its own style position.
        /// </summary>
        internal virtual IReadOnlyList<BaseModifier> LayerChildren => Children;
        ...
        /// <paramref name="hasRanges"/> is <see langword="false"/>; an unowned instance publishes the
        /// mesh change of <paramref name="member"/> instead, so an authoring-time edit replays into
        /// every runtime copy rather than replacing it.
        /// </summary>
        protected void MarkRenderDirty(bool hasRanges, StateMember member = default)
        {
            if (uniText == null) MarkMeshDirty(member);
