...
namespace LightSide
{
    ...
    public sealed partial class ModifierGraphPreset : ScriptableObject, IModifierChangeSink
    {
        ...
        [Tooltip("Modifier, or a Composite of modifiers, that every use of this preset applies.")]
        ...
        /// <summary>Copies this preset's root graph for one component: a detached managed graph the asset never observes.</summary>
        internal static BaseModifier CreateRuntimeRoot(ModifierGraphPreset source)
        {
            if (source == null) throw new ArgumentNullException(nameof(source));
            source.Validate();
            return StateCopy.Copy(source.Root) ?? throw new InvalidOperationException(
                $"Modifier graph preset '{source.name}' has no root.");
        }
    ...
    public sealed partial class ModifierGraphModifier : BaseModifier
    {
        ...
        [Tooltip("Modifier Graph Preset asset applied through this node; each text gets its own private copy of it.")]
        ...
        [NonSerialized] private int runtimeVersion;
        ...
        private void ApplyPresetChange()
        {
            ...
            if (Owner is not null) SynchronizeRuntimeGraph(preset, IsInitialized);
            else DisposeRuntimeGraph();
        ...
        internal override void SetOwner(UniTextBase owner, IModifierChangeSink sink = null)
        {
            ...
            try
            {
                ...
                if (preset != null && (runtimeRoot == null || runtimeVersion != preset.Version))
                    ResyncRuntimeGraph(preset);
        ...
        private void DisposeRuntimeGraph()
        {
            ...
            ReleaseRuntimeGraph(previousRoot);
        }

        private void OnPresetChanged(ModifierGraphPreset source,
            in ModifierGraphPresetChange change)
        {
            ...
            RefreshAuthoring();
            if (!ResyncRuntimeGraph(source)) MarkStructureChanged();
        }

        private bool ResyncRuntimeGraph(ModifierGraphPreset source)
        {
            var version = source.Version;
            if (runtimeRoot != null && StateCopy.TryReplay(runtimeRoot, source.Root))
            {
                runtimeVersion = version;
                return true;
            }
            SynchronizeRuntimeGraph(source, IsInitialized);
            return false;
        }
        ...
        {
            var version = source.Version;
            var candidateRoot = BuildRuntimeGraph(source, prepare);
            var previousRoot = runtimeRoot;
        ...
        private BaseModifier BuildRuntimeGraph(ModifierGraphPreset source, bool prepare)
        {
            if (source == null)
                throw new InvalidOperationException("ModifierGraphModifier requires a preset.");
            var candidateRoot = ModifierGraphPreset.CreateRuntimeRoot(source);
            try
            {
            ...
            {
                ReleaseRuntimeGraph(candidateRoot);
                throw;
            }
        }

        private static void ReleaseRuntimeGraph(BaseModifier graphRoot)
        {
            if (graphRoot == null) return;
            if (!MainThread.IsCurrent)
            {
                MainThread.Post(() => ReleaseRuntimeGraph(graphRoot));
                return;
            }
            try
            {
