...
namespace LightSide
{
    ...
    /// per-glyph mode rides the vertex stream (UV1.w), so one material renders them all. A colour glyph
    /// drawn from a texture outside the atlases is never copied: no text material can render it.</item>
    ...
    public abstract class SubMeshModifier : BaseModifier, ILayer
    {
        ...
        {
            var gen = uniText.MeshGenerator;
            if (gen.HasExternalFace || !ShouldIncludeCurrentGlyph()) return;
