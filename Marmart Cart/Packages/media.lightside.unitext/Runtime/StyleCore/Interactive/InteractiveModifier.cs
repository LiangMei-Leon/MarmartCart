...
namespace LightSide
{
    ...
    public partial class InteractiveModifier : BaseModifier, IModifierCommitChanges
    {
        ...
        [NonSerialized] private List<BaseModifier> ruleTemplates;

        internal override IReadOnlyList<BaseModifier> LayerChildren => ruleTemplates;

        private void ApplyRulesChange()
        {
            RefreshAuthoring();
            if (!IsInitialized) return;
            var registered = ResolveRegisteredRules();
            var runtime = UniTextRanges.For(uniText);
            runtime.Unregister(this);
            BindRuleChanges();
            CollectRuleTemplates();
            runtime.Register(this, registered);
        }

        private void CollectRuleTemplates()
        {
            ruleTemplates ??= new List<BaseModifier>();
            ruleTemplates.Clear();
            for (var i = 0; i < rules.Length; i++)
                if (rules[i] is ModifierRule { ModifierTemplate: { } template })
        ...
        private void ApplyActionsChange()
        {
            ...
            if (IsInitialized)
                ModifierGraphValidator.ThrowIfInvalid(ModifierGraphValidator.FindRoot(this),
                    validateConfiguration: !IsAuthoring);
        }

        /// <summary>
        /// Validates the owning graph and returns the rules the runtime binds: every rule outside
        /// authoring, only the completely configured ones while the document is being authored.
        /// </summary>
        private IReadOnlyList<RangeStateRule> ResolveRegisteredRules()
        {
            var root = ModifierGraphValidator.FindRoot(this);
            if (!IsAuthoring)
            {
                ModifierGraphValidator.ThrowIfInvalid(root);
                return rules;
            }
            var configured = new List<RangeStateRule>(rules.Length);
            ModifierGraphValidator.CollectConfiguredRules(root, rules, configured);
            return configured.Count == rules.Length ? rules : configured;
        }
        ...
        protected override void OnEnable()
        {
            ...
            BindRuleChanges();
            CollectRuleTemplates();
            UniTextRanges.For(uniText).Register(this, registered);
        }

        protected internal override void PrepareForParallel()
        {
            BindRuleChanges();
            if (ruleTemplates == null) return;
            for (var i = 0; i < ruleTemplates.Count; i++)
                ruleTemplates[i].PrepareForParallel();
        }
