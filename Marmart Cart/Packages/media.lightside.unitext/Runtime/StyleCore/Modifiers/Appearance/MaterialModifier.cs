...
namespace LightSide
{
    ...
    /// <remarks>
    /// <para>
    /// The custom shader must include <c>LightSide_Custom.cginc</c> — see
    /// <c>Assets/Create/UniText/Custom Effect</c> for a working template. The glyph atlases are
    ...
    public partial class MaterialModifier : SubMeshModifier, IModifierCommitChanges
    {
        ...
        [Tooltip("Custom material to apply to the range. The shader must include LightSide_Custom.cginc.")]
        ...
        /// <summary>Optional outward expansion of emitted glyph quads, in glyph heights.</summary>
        [Tooltip("Expand each glyph quad outward by this fraction of the glyph height so shader effects " +
                 "(glow, edge, etc.) aren't clipped by the glyph's tight bounding box. The atlas reserves " +
                 "matching rim, up to the signed-distance spread it can store. Negative = read from shader's " +
                 "_LightSideMeshPadding property (shader-driven default).")]
        ...
        [Tooltip("Tint multiplied into the vertex color the custom material receives.")]
        ...
        {
            if (activePadding > 0f)
                ReserveOutwardReach(vertexStart, gen);
        ...
        /// <summary>
        /// Reserves the rim the padded quad reaches into: the tier request re-rasterizes the tile with valid
        /// field out there, and the expansion makes that field visible. Both are capped by the atlas's
        /// signed-distance spread and by the room the face dilate left. A colour face carries atlas UV0
        /// instead of a glyph-local field and stays unpadded.
        /// </summary>
        private void ReserveOutwardReach(int vertexStart, UniTextMeshGenerator gen)
        {
            if (gen.font is { IsColor: true }) return;
            ...
            var reach = activePadding < padGlyph ? activePadding : padGlyph;
            ...
            if (delta > 0f) ExpandSubMeshQuad(vertexStart, delta);
        }
