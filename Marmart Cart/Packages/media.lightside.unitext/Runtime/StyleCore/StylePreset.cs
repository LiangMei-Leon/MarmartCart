...
namespace LightSide
{
    ...
    public partial class StylePreset : ScriptableObject, IStyleChangeSink
    {
        ...
        /// <summary>Copies every style of <paramref name="source"/> for one component: detached managed graphs the asset never observes, sharing managed references only among themselves.</summary>
        internal static Style[] CreateRuntimeStyles(StylePreset source)
        {
            ...
            var result = new Style[source.Styles.Count];
            for (var i = 0; i < result.Length; i++) result[i] = context.Copy(source.Styles[i]);
            return result;
        }
