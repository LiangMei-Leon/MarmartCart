    ...
    internal static class EmbeddedFontCatalog
    {
        ...
        private sealed class CachedFontSource : FontSource
        {
            ...
            internal bool HasBacking
                => fetchToken == 0
                   || FontFileCache.FileHasLength(finalPath, rawLength)
            ...
            internal void AdoptCompressed(byte[] data)
            {
                ...
                try
                {
                    ...
                    FontFileCache.PublishTemporaryFile(temporaryPath, compressedPath, data.Length, sourceId);
                }
                finally
                {
            ...
            {
                if (FontFileCache.FileHasLength(finalPath, rawLength)) return;
            ...
            private void Extract(Stream input)
            {
                ...
                try
                {
                    ...
                    FontFileCache.PublishTemporaryFile(temporaryPath, finalPath, rawLength, sourceId);
                }
                finally
                {
                    if (File.Exists(temporaryPath)) File.Delete(temporaryPath);
