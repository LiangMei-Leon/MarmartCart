...
namespace LightSide
{
        ...
        IInputBehaviorChangeSink
    {
        ...
        private readonly List<BehaviorPresetBinding> runtimeBehaviorPresets = new();

        private sealed class BehaviorPresetBinding
        {
            internal readonly InputBehaviorPreset source;
            internal InputBehavior[] authored;
            internal InputBehavior[] behaviors;
            internal int version;

            internal BehaviorPresetBinding(InputBehaviorPreset source)
            {
                this.source = source;
                version = source.Version;
                authored = AuthoredBehaviors(source);
                behaviors = source.CreateRuntimeBehaviors();
            }

            /// <summary>True when the preset published a transition this binding did not observe.</summary>
            internal bool IsStale => version != source.Version;
        }
...
#pragma warning restore 414
        ...
        private void EnableBehaviors()
        {
            ...
            ReconcileBehaviorPresetBindings(0);
            for (var i = 0; i < runtimeBehaviorPresets.Count; i++)
                if (runtimeBehaviorPresets[i].IsStale) ResyncBehaviorPreset(i, false);
        ...
        private void DisableBehaviors()
        {
            ...
            for (var i = 0; i < runtimeBehaviorPresets.Count; i++)
                DetachBehaviors(runtimeBehaviorPresets[i].behaviors);
        ...
        private void EnableCopyBehaviors(BehaviorPresetBinding binding)
        {
            var presetBehaviors = binding.behaviors;
            for (int j = 0; j < presetBehaviors.Length; j++)
            {
        ...
        private static void DetachBehaviors(InputBehavior[] presetBehaviors)
        {
            for (var i = presetBehaviors.Length - 1; i >= 0; i--) presetBehaviors[i].SetOwner(null);
        }

        private static InputBehavior[] AuthoredBehaviors(InputBehaviorPreset preset)
        {
            var behaviors = preset.Behaviors;
            var result = new InputBehavior[behaviors.Count];
            for (var i = 0; i < result.Length; i++) result[i] = behaviors[i];
            return result;
        }
            ...
            in InputBehaviorPresetChange change)
        {
            ...
            if (!TryReplayBehaviorPresetChange(presetIndex, in change))
                ResyncBehaviorPreset(presetIndex, true);
            runtimeBehaviorPresets[presetIndex].version = preset.Version;
        }
            ...
            in InputBehaviorPresetChange change)
        {
            ...
            var behaviorIndex = ReferenceIdentity.IndexOf(binding.authored, change.Behavior);
            if ((uint)behaviorIndex >= (uint)binding.behaviors.Length) return false;
            var targetBehavior = binding.behaviors[behaviorIndex];
        ...
        /// <summary>
        /// Aligns a binding's runtime behaviors with the preset's current authored behaviors: a behavior
        /// whose authored source is still present replays that source in place, any other gets a fresh
        /// copy, and a live binding restarts the hook-order suffix from the first behavior whose identity
        /// or position changed.
        /// </summary>
        private void ResyncBehaviorPreset(int presetIndex, bool live)
        {
            var binding = runtimeBehaviorPresets[presetIndex];
            var preset = binding.source;
            var version = preset.Version;
            var authored = AuthoredBehaviors(preset);
            var previous = binding.behaviors;
            var current = new InputBehavior[authored.Length];
            var first = Math.Min(previous.Length, authored.Length);
            var context = new StateCopyContext();
            for (var i = 0; i < authored.Length; i++)
            {
                var index = ReferenceIdentity.IndexOf(binding.authored, authored[i]);
                if (index >= 0 && context.TryReplay(previous[index], authored[i]))
                {
                    current[i] = previous[index];
                    if (index != i) first = Math.Min(first, i);
                    continue;
                }
                current[i] = context.Copy(authored[i]);
                first = Math.Min(first, i);
            }
            var structural = first < Math.Max(previous.Length, current.Length);
            if (live && structural) DisableBehaviorPresetSuffix(presetIndex, first);
            for (var i = previous.Length - 1; i >= 0; i--)
                if (ReferenceIdentity.IndexOf(current, previous[i]) < 0) previous[i].SetOwner(null);
            binding.authored = authored;
            binding.behaviors = current;
            binding.version = version;
            if (live && structural) EnableBehaviorPresetSuffix(presetIndex, first);
        }
        ...
        private void ReconcileBehaviorPresetCopies()
        {
            ...
            ReconcileBehaviorPresetBindings(prefix);
            EnableBehaviorPresetCopiesFrom(prefix);
            RefreshBehaviorPresetSubscriptions();
        }

        private void ReconcileBehaviorPresetBindings(int firstChangedIndex)
        {
            for (var targetIndex = firstChangedIndex; targetIndex < behaviorPresets.Count; targetIndex++)
            {
                ...
                runtimeBehaviorPresets.Insert(targetIndex, new BehaviorPresetBinding(source));
            }

            while (runtimeBehaviorPresets.Count > behaviorPresets.Count)
                RemoveBehaviorPresetBindingAt(runtimeBehaviorPresets.Count - 1);
        }
        ...
        private void DisableBehaviorPresetCopiesFrom(int index)
        {
            ...
            {
                var presetBehaviors = runtimeBehaviorPresets[i].behaviors;
                for (var j = presetBehaviors.Length - 1; j >= 0; j--)
        ...
        private void EnableBehaviorPresetCopiesFrom(int index)
        {
            for (var i = index; i < runtimeBehaviorPresets.Count; i++)
                EnableCopyBehaviors(runtimeBehaviorPresets[i]);
        }

        private void RemoveBehaviorPresetBindingAt(int index)
        {
            DetachBehaviors(runtimeBehaviorPresets[index].behaviors);
            ...
            IStateMemberReplay source, StateMember member, bool structural)
        {
            ...
            {
                var behaviorIndex = ReferenceIdentity.IndexOf(
                    runtimeBehaviorPresets[presetIndex].behaviors, behavior);
        ...
        private void DisableBehaviorPresetSuffix(int presetIndex, int behaviorIndex)
        {
            for (var i = runtimeBehaviorPresets.Count - 1; i >= presetIndex; i--)
            {
                ...
                for (var j = presetBehaviors.Length - 1; j >= first; j--)
                    presetBehaviors[j].Disable();
            }
        }
        ...
        private void EnableBehaviorPresetSuffix(int presetIndex, int behaviorIndex)
        {
            for (var i = presetIndex; i < runtimeBehaviorPresets.Count; i++)
            {
                var presetBehaviors = runtimeBehaviorPresets[i].behaviors;
                var first = i == presetIndex ? behaviorIndex : 0;
                for (var j = first; j < presetBehaviors.Length; j++)
                {
        ...
        /// <summary>
        /// Returns the first attached behavior of type <typeparamref name="T"/>, or <see langword="null"/>.
        /// A preset-sourced behavior is this editor's own copy: it survives disable/enable and takes the
        /// preset's later edits in place; only replacing its authored node replaces the copy.
        /// </summary>
