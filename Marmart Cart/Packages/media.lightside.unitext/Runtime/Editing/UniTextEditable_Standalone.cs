...
namespace LightSide
{
        ...
        IUpdateSelectedHandler
    {
        ...
        /// them by pointer kind to the touch gesture recogniser or the desktop autoscroll tracker.
        /// Touch holds are claimed from the core (<see cref="UniTextBase.longPressClaimed"/>) — the
        /// recogniser presents their long-press flow.
        /// </summary>
        private void SubscribeTextEvents()
        {
            var text = TextComponent;
            text.longPressClaimed = true;
        ...
        /// <summary>
        /// Press: a touch press opens the touch session and feeds the gesture recogniser; a mouse / pen
        /// press closes any touch session, activates the editor and runs the shared press gesture
        /// (single → caret with hit affinity, double → word + word-drag, triple → line; Shift extends).
        /// Any press breaks undo coalescing and resets the vertical-movement column anchor.
        /// </summary>
        ...
        private void OnTextDragStarted(TextPointerEvent evt)
        {
            if (evt.Kind == PointerKind.Touch)
            {
                if (touchActive) touchGesture.OnPointerMove(evt.ScreenPosition, Time.unscaledTime);
                return;
            }
        ...
        /// A touch press is the exception: the EventSystem selects on pointer-DOWN, but touch focus must
        /// wait for the gesture recogniser's tap (press and release in place) so a press that turns into a
        /// scroll or a pass-through drag never focuses. Programmatic focus activates directly through
        /// <see cref="Activate"/>.
        /// </summary>
        void ISelectHandler.OnSelect(BaseEventData eventData)
        {
            if (eventData is PointerEventData pointer &&
                UniTextBase.ResolvePointerKind(pointer) == PointerKind.Touch) return;
            Activate();
        }
