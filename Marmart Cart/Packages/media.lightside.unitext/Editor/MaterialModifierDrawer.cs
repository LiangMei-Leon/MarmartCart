...
namespace LightSide
{
    ...
    /// fields. Auto-discovers per-text shader parameters marked with the <see cref="ShaderIds.Custom.InstPrefix"/>
    /// prefix and <c>[HideInInspector]</c>, and draws them as typed controls (slider / float / color / vector)
    ...
    internal sealed class MaterialModifierDrawer : IManagedReferenceDrawer
    {
        ...
        private static InstLayout ResolveInstLayout(Shader shader)
        {
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                if (!name.StartsWith(ShaderIds.Custom.InstPrefix, System.StringComparison.Ordinal)) continue;
                var flags = shader.GetPropertyFlags(i);
                if ((flags & ShaderPropertyFlags.HideInInspector) == 0) continue;
                if (!TryParseSlot(name.Substring(ShaderIds.Custom.InstPrefix.Length), out var channel, out var componentIdx)) continue;
