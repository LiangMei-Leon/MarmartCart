...
namespace LightSide
{
    internal abstract partial class UniTextBaseEditor
    {
        ...
        private VisualElement styleIssues;
        ...
        private VisualElement CreateStyleSection()
        {
            ...
            styleIssues = InspectorVisuals.CreateStack();
            presetButtons.Clear();
        ...
        private void RefreshStyleState()
        {
            ...
            InspectorVisuals.ClearContent(styleIssues);
            for (var targetIndex = 0; targetIndex < targets.Length; targetIndex++)
            {
                var text = (UniTextBase)targets[targetIndex];
                graphIssues.Clear();
                ModifierGraphValidator.Validate(text, graphIssues);
                ModifierGraphIssueRenderer.Append(styleIssues, graphIssues,
                    targets.Length == 1 ? null : text.name);
            }
        }
