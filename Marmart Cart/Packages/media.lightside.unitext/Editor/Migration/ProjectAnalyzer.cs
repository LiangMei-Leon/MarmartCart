...
namespace LightSide
{
    ...
    internal class ProjectAnalyzer
    {
        ...
        const string TmpAsmRef = "\"" + MigrationMapping.TmpAssemblyName + "\"";
        ...
        static readonly Regex bareGuidRegex = new(@"[a-f0-9]{32}", RegexOptions.Compiled);
        ...
        static readonly byte[] assemblyMarker = Ascii(MigrationMapping.TmpAssemblyName);
        ...
        void ScanTmpAsset(string path, string content, FileScan scan)
        {
            foreach (var assetGuid in MigrationMapping.TmpAssetGuids)
            {
                ...
                if (assetGuid == MigrationMapping.TmpFontAssetGuid)
                {
                    ...
                    scan.FontSourceGuid =
                        ReadGuidField(content, MigrationMapping.SourceFontField) ??
                        ReadGuidField(content, MigrationMapping.LegacySourceFontField);
                    scan.HasFont = true;
                    ...
                    var sourcePath = TryFindTtfSource(scan.FontSourceGuid, fontName);
        ...
        /// <summary>The GUID a bare YAML scalar field holds, or null when the field is absent or empty.</summary>
        static string ReadGuidField(string content, string field)
        {
            int at = FindFieldLine(content, field);
            if (at < 0) return null;
            int lineEnd = content.IndexOf('\n', at);
            if (lineEnd < 0) lineEnd = content.Length;
            var match = bareGuidRegex.Match(content, at, lineEnd - at);
        ...
        /// <summary>
        /// The font file a TMP font asset was built from: the indexed file its recorded source
        /// GUID names, otherwise the indexed file whose name the asset's file name contains, the
        /// longest such name winning. Null when neither answers.
        /// </summary>
        string TryFindTtfSource(string sourceGuid, string fontName)
        {
            if (sourceGuid != null)
            {
                for (int i = 0; i < fontSources.Count; i++)
                    if (fontSources[i].guid == sourceGuid) return fontSources[i].path;
            }
        ...
        {
            if (content.IndexOf(TmpAsmRef, StringComparison.Ordinal) < 0) return;
            ...
            scan.Findings.Add(new MigrationFinding
            {
                ...
                complexity = MigrationComplexity.Simple,
                details = $"References {MigrationMapping.TmpAssemblyName} assembly",
            });
        }
        ...
        void Integrate(FileEntry entry, FileScan scan)
        {
            ...
            if (scan.HasFont)
            {
                DiscoveredFonts.Add(new FontMappingEntry
                {
                    ...
                    sourceTtfPath = TryFindTtfSource(scan.FontSourceGuid,
            ...
            ScannedFiles.Add(new ScannedFileRecord
            {
                ...
                fontSourceGuid = scan.FontSourceGuid,
        ...
        void ScanCompiledAssemblies()
        {
            ...
            foreach (var asm in LoadedAssemblies.All())
            {
                var asmName = asm.GetName().Name;
                if (asmName.Contains("TextMeshPro")) continue;
                if (asm.IsDynamic) continue;

                var location = LoadedAssemblies.PathOf(asm);
                if (string.IsNullOrEmpty(location)) continue;
                if (!plugins.TryGetValue(NormalizePath(Path.GetFullPath(location)),
                        out var assetPath)) continue;
        ...
        /// <summary>
        /// Every precompiled plugin the asset database knows, by absolute filesystem path.
        /// Identity comes from the asset database, so Unity's own
        ...
        readonly struct FontSource
        {
            ...
            public FontSource(string path, string guid)
            {
                this.path = path;
                this.guid = guid;
        ...
        readonly struct CachedFile
        {
            ...
            public FileScan ToScan()
            {
                ...
                scan.FontSourceGuid = record.fontSourceGuid;
