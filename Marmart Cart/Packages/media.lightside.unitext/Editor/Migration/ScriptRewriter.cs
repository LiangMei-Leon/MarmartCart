...
namespace LightSide
{
    ...
    internal static class ScriptRewriter
    {
        ...
        private static readonly HashSet<string> declarationModifiers = new(StringComparer.Ordinal)
        {
            ...
            "volatile", "async", "event", "params", "using", "return", "throw", "yield",
        };
        ...
        private static HashSet<string> LightSideTypeNames()
        {
            ...
            var assemblies = LoadedAssemblies.All();
        ...
        public static List<ScriptReplacement> Analyze(string content, ScriptMemberIndex projectIndex = null)
        {
            var pass = new Pass(content, projectIndex);
            pass.Run();
            return pass.Results;
        }

        /// <summary>Records every TMP-typed member <paramref name="content"/> declares into <paramref name="index"/>.</summary>
        public static void Index(string content, ScriptMemberIndex index)
        {
            new Pass(content, null).IndexDeclarations(index);
        }

        /// <summary>One TMP-typed member, under the type that declares it.</summary>
        private struct MemberDeclaration
        {
            public string owner;
            public string name;
            public MigrationMapping.TmpTypeKind kind;
            public bool isCollection;
        }

        /// <summary>A type body, and the brace depth inside it.</summary>
        private struct TypeSpan
        {
            public string name;
            public int start;
            public int end;
        ...
        private sealed class Pass
        {
            ...
            private readonly Dictionary<string, string> declaredTypes = new(StringComparer.Ordinal);
            private readonly Dictionary<string, string> elementTypes = new(StringComparer.Ordinal);
            private readonly HashSet<string> contestedNames = new(StringComparer.Ordinal);
            private readonly List<MemberDeclaration> memberDeclarations = new();
            private readonly ScriptMemberIndex projectIndex;
            private List<TypeSpan> typeSpans;
            ...
            public Pass(string source, ScriptMemberIndex projectIndex)
            {
                this.source = source;
            ...
            private void CollectDeclarations()
            {
                ...
                for (var i = 0; i < identifiers.Count; i++)
                {
                    ...
                    {
                        if (IsConstructedType(start)) continue;
                        DeclareFromType(end, mapping.Kind);
                        RecordDeclaration(start, end, word, mapping.Kind);
                        continue;
                    }
                    if (MigrationMapping.UniTextTypeKinds.TryGetValue(word, out var migrated))
                    {
                        if (!IsConstructedType(start)) RecordDeclaration(start, end, word, migrated);
                        continue;
                    }
                    if (word == "var") { DeclareFromVar(end); continue; }
                    if (word == "foreach") { DeclareFromForeach(end); continue; }
                    if (!declarationStoppers.Contains(word) && !declarationModifiers.Contains(word) &&
                        !IsMemberAccess(start) && !IsConstructedType(start))
                        RecordDeclaration(start, end, word, null);
                }
            }

            /// <summary>
            /// Maps every type body in the file, so a declaration can name the type that holds it.
            /// Bodies nest, and the narrowest one containing a position owns it.
            /// </summary>
            private void CollectTypeSpans()
            {
                ...
                for (var i = 0; i < source.Length; i++)
                {
                    ...
                    if (IsIdentifierStart(current) && (i == 0 || !IsIdentifierPart(source[i - 1])))
                    {
                        var word = ReadIdentifierAt(i, out var wordEnd);
                        if (word == "class" || word == "struct" || word == "interface" ||
                            word == "record")
                        {
                            var nameStart = NextSignificant(wordEnd);
                            var name = nameStart < source.Length
                                ? ReadIdentifierAt(nameStart, out _)
                                : null;
                            if (name != null) { pendingName = name; pendingDepth = depth; }
                        }
                        i = wordEnd - 1;
                        continue;
                    }
                    ...
                    {
                        depth++;
                        if (pendingName != null && depth == pendingDepth + 1)
                        {
                            typeSpans.Add(new TypeSpan
                            {
                                name = pendingName,
                                start = i,
                                end = source.Length,
                    ...
                    if (current == '}')
                    {
                        if (open.Count > 0 && typeSpans[open.Peek()].bodyDepth == depth)
                        {
                            var index = open.Pop();
                            var span = typeSpans[index];
                            span.end = i;
                            typeSpans[index] = span;
                        }
                        depth--;
                        continue;
                    }

                    if (current == ';') pendingName = null;
                }
            }

            /// <summary>Name of the narrowest type body containing <paramref name="position"/>, or null outside every type.</summary>
            private string OwnerTypeAt(int position)
            {
                string owner = null;
                var narrowest = int.MaxValue;
                for (var i = 0; i < typeSpans.Count; i++)
                {
                    var span = typeSpans[i];
                    if (position < span.start || position > span.end) continue;
                    var width = span.end - span.start;
            ...
            /// <summary>Adds this file's TMP-typed members to <paramref name="index"/>, under the types that declare them.</summary>
            internal void IndexDeclarations(ScriptMemberIndex index)
            {
                CollectDeclarations();
                for (var i = 0; i < memberDeclarations.Count; i++)
                {
                    var entry = memberDeclarations[i];
                    index.Add(entry.owner, entry.name, entry.kind, entry.isCollection);
                }
            }
            ...
            private void DeclareFromType(int typeEnd, MigrationMapping.TmpTypeKind kind)
                => ForEachDeclarator(typeEnd, (name, nameStart, isCollection) =>
                {
            ...
            /// <summary>
            /// Visits every declarator a type token introduces, with where the name starts and
            /// whether the type was written as a collection.
            /// </summary>
            private void ForEachDeclarator(int typeEnd, Action<string, int, bool> visit,
                bool strict = false)
            {
                ...
                while (true)
                {
                    ...
                    if (strict && !ClosesDeclarator(next)) return;
                    visit(name, i, isCollection);
            ...
            /// <summary>Whether the character at <paramref name="index"/> can end a declarator.</summary>
            private bool ClosesDeclarator(int index)
            {
                if (index >= source.Length) return false;
                var current = source[index];
                if (current == ';' || current == '=' || current == ',' ||
                    current == ')' || current == '{') return true;
                return ReadIdentifierAt(index, out _) == "in";
            }

            /// <summary>
            /// Binds what a type token declares: every name to the type it was written with, and a
            /// TMP-typed name to the type that declares it, for the project index.
            /// </summary>
            private void RecordDeclaration(int typeStart, int typeEnd, string typeName,
                MigrationMapping.TmpTypeKind? kind)
            {
                var owner = kind.HasValue ? OwnerTypeAt(typeStart) : null;
                var generic = NextSignificant(typeEnd);
                if (generic < source.Length && source[generic] == '<')
                {
                    var closeAngle = MatchForward(generic, '<', '>');
                    if (closeAngle >= 0) RecordGenericDeclaration(generic, closeAngle, typeName);
                    return;
                }

                ForEachDeclarator(typeEnd, (name, nameStart, isCollection) =>
                {
                    if (declaredTypes.TryGetValue(name, out var boundType))
                    {
                        if (!string.Equals(boundType, typeName, StringComparison.Ordinal))
                            contestedNames.Add(name);
                    }
                    else declaredTypes[name] = typeName;
                    if (isCollection && !elementTypes.ContainsKey(name)) elementTypes[name] = typeName;
                    if (owner != null)
                        memberDeclarations.Add(new MemberDeclaration
                        {
                            owner = owner,
                            name = name,
                            kind = kind.Value,
            ...
            /// <summary>
            /// Binds a name declared with a generic collection type, taking the element type from
            /// the last type argument so an indexer read resolves through the project index.
            /// </summary>
            private void RecordGenericDeclaration(int openAngle, int closeAngle, string typeName)
            {
                var inner = source.Substring(openAngle + 1, closeAngle - openAngle - 1);
                var comma = inner.LastIndexOf(',');
                if (comma >= 0) inner = inner.Substring(comma + 1);
                var qualifier = inner.LastIndexOf('.');
                if (qualifier >= 0) inner = inner.Substring(qualifier + 1);
                var element = inner.Trim();
                if (element.Length == 0 || !IsIdentifierStart(element[0])) return;

                var nameStart = NextSignificant(closeAngle + 1);
                if (nameStart >= source.Length) return;
                var name = ReadIdentifierAt(nameStart, out var nameEnd);
                if (name == null || declarationStoppers.Contains(name)) return;
                if (!ClosesDeclarator(NextSignificant(nameEnd))) return;

                if (!elementTypes.ContainsKey(name)) elementTypes[name] = element;
            }

            /// <summary>
            /// The type name an initializer states outright: a construction, a call's single type
            /// argument, or the element type of a collection read by index. Null for every other
            /// shape, and for a read that continues past the one it names.
            /// </summary>
            private string InferTypeName(int start, int end)
            {
                var cursor = NextSignificant(start);
                if (cursor >= end || cursor >= source.Length) return null;

                if (ReadIdentifierAt(cursor, out var wordEnd) == "new")
                {
                    var typeStart = NextSignificant(wordEnd);
                    return typeStart < end ? ReadIdentifierAt(typeStart, out _) : null;
                }

                for (var i = cursor; i < end && i < source.Length; i++)
                {
                    if (!isCode[i]) continue;
                    if (source[i] == '<')
                    {
                        var closeAngle = MatchForward(i, '<', '>');
                        if (closeAngle < 0 || closeAngle >= end) return null;
                        var call = NextSignificant(closeAngle + 1);
                        if (call >= source.Length || source[call] != '(') return null;
                        var closeCall = MatchForward(call, '(', ')');
                        if (closeCall < 0 || NextSignificant(closeCall + 1) < end) return null;
                        var argument = source.Substring(i + 1, closeAngle - i - 1).Trim();
                    ...
                    if (source[i] == '[')
                    {
                        var closeBracket = MatchForward(i, '[', ']');
                        if (closeBracket < 0 || NextSignificant(closeBracket + 1) < end) return null;
                        var collection = ReadIdentifierBefore(PreviousSignificant(i - 1), out _);
                        return collection != null && elementTypes.TryGetValue(collection, out var element)
                            ? element
                            : null;
                    }
                }
            ...
            /// <summary>
            /// The kind the project index gives <paramref name="member"/> where it is read off
            /// another object whose type this file declares. Null where either end is unknown.
            /// </summary>
            private MigrationMapping.TmpTypeKind? FromProjectIndex(string member, int memberStart,
                bool isCollection)
            {
                if (projectIndex == null) return null;
                var dot = PreviousSignificant(memberStart - 1);
                if (dot < 0 || source[dot] != '.') return null;

                var ownerEnd = PreviousSignificant(dot - 1);
                if (ownerEnd < 0) return null;
                string ownerType;
                if (source[ownerEnd] == ']')
                {
                    var openBracket = MatchBackward(ownerEnd, '[', ']');
                    if (openBracket < 0) return null;
                    var collection = ReadIdentifierBefore(PreviousSignificant(openBracket - 1), out _);
                    if (collection == null || contestedNames.Contains(collection) ||
                        !elementTypes.TryGetValue(collection, out ownerType))
                        return null;
                }
                else
                {
                    var ownerName = ReadIdentifierBefore(ownerEnd, out _);
                    if (ownerName == null || contestedNames.Contains(ownerName) ||
                        !declaredTypes.TryGetValue(ownerName, out ownerType))
                        return null;
                }
                return projectIndex.TryGetMember(ownerType, member, isCollection, out var kind)
                    ? kind
            ...
            private void DeclareFromVar(int varEnd)
            {
                ...
                var inferred = InferTypeName(assign + 1, end);
                if (inferred != null && !declaredTypes.ContainsKey(name)) declaredTypes[name] = inferred;
                if (kind.HasValue)
                {
            ...
            private MigrationMapping.TmpTypeKind? ResolveReceiver(int dotIndex)
            {
                ...
                if (source[i] == ']')
                {
                    ...
                    if (i < 0) return null;
                    var indexed = ReadIdentifierBefore(i, out var indexedStart);
                    if (indexed == null) return null;
                    if (collections.TryGetValue(indexed, out var elementKind)) return elementKind;
                    return FromProjectIndex(indexed, indexedStart, true);
                }
                ...
                var projectKind = FromProjectIndex(name, nameStart, false);
                if (projectKind.HasValue) return projectKind;
