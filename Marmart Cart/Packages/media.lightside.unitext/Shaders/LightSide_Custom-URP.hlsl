...
struct LightSide_appdata
{
    ...
    float4 texcoord1 : TEXCOORD1;  // x = aspect + 1024*(deformRow+1) (LightSideDeform.hlsl; DeformModifier deforms only the base surfaces, so a custom effect reads the lane and samples undeformed), y = faceDilate, z = cluster, w = intra-glyph X fraction (0..1) + 2*glyphMode
...
// gamma-space project compiles the call away. LightSideVert already applies it.
