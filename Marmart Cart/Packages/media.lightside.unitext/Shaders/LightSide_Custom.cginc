...
//
// Color space — wrap v.color in LightSideGammaToLinearIfNeeded(v.color); LightSideVert already
// does. The colour stream is the authored sRGB bytes on every world mesh, and on a Canvas mesh
// only while the Canvas defers its own conversion to the shader (_UIVertexColorAlwaysGammaSpace);
// a linear-space project decodes those, a gamma-space project passes them through. Both face and
// effect (outline / shadow / extrude) quads ship their colour in v.color — effect quads have
// alpha pre-multiplied by face alpha on the CPU side, so face alpha modulation flows through
// automatically.
...
struct LightSide_appdata
{
    ...
    float4 texcoord1 : TEXCOORD1; // x = aspect + 1024*(deformRow+1) (LightSideDeform.hlsl; DeformModifier deforms only the base surfaces, so a custom effect reads the lane and samples undeformed), y = faceDilate, z = cluster index, w = intra-glyph X fraction (0..1) + 2*glyphMode (LightSideGlyphMode)
...
#else
    bool sRGBEncoded = _UIVertexColorAlwaysGammaSpace != 0;
#endif
