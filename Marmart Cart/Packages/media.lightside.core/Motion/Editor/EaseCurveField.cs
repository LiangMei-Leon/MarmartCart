...
namespace LightSide
{
    ...
    public sealed class EaseCurveField : VisualElement
    {
        ...
        private readonly EaseCurveOptions options;
        private readonly ViewNavigator navigator = new(Rect.MinMaxRect(0f, 0f, 1f, 1f));
        ...
        public bool IsBusy => pointerHeld || navigator.IsBusy || (editor?.IsBusy ?? false);

        /// <summary>Binds the canvas to an <see cref="Ease"/> field on every selected target.</summary>
        /// <exception cref="ArgumentNullException"><paramref name="binding"/> is <see langword="null"/>.</exception>
        /// <summary>Creates a canvas over the bound ease as the timing curve it is by default.</summary>
        public EaseCurveField(SerializedPropertyBinding binding)
            : this(binding, EaseCurveOptions.Timing) { }

        /// <summary>Creates a canvas over the bound ease, presenting it as <paramref name="options"/> asks.</summary>
        public EaseCurveField(SerializedPropertyBinding binding, in EaseCurveOptions options)
        {
            ...
            navigator.Changed += RefreshAxis;
            RegisterCallback<CustomStyleResolvedEvent>(OnStylesResolved);
            RegisterCallback<GeometryChangedEvent>(_ => RefreshAxis());
        ...
        public string ReadoutText
        {
            get
            {
                ...
                return value.Kind == EaseKind.Keyed
                    ? "Double-click adds a key · middle-drag pans · Ctrl zooms · F frames"
                    : string.Empty;
            }
        }
        ...
        {
            working = knots;
            points = new EasePointSet(working, options.PinEndValues && value.Kind == EaseKind.Cubic, null);
        ...
        private void PopulateMenu(DropdownMenu menu)
        {
            ...
            if (binding.FindSerializedProperty() is { } property)
                ContextualPropertyMenu.Append(menu, property);
        }
        ...
        private void OnPointerDown(PointerDownEvent evt)
        {
            ...
            if (navigator.HandlePointerDown(evt.button, evt.modifiers, evt.localPosition, surface))
            {
                Focus();
        ...
        private void OnPointerMove(PointerMoveEvent evt)
        {
            lastModifiers = evt.modifiers;
            surface.SetPointer(evt.localPosition);
            if (navigator.HandlePointerMove(evt.localPosition))
            {
                Consume(evt);
                return;
            }
        ...
        {
            if (evt.keyCode == KeyCode.F && evt.modifiers == EventModifiers.None)
            {
                navigator.Reset();
                evt.StopPropagation();
                return;
        ...
        {
            var canvas = contentRect;
            if (canvas.width <= 4f || canvas.height <= 4f) return;

            var boundsRect = navigator.View;
            surface.SetView(canvas, boundsRect, Margin, preserveAspect: false);
        ...
        /// <summary>
        /// Maps the shown span onto the canvas and writes the axis labels. Runs outside the paint pass: a
        /// label is an element, and an element cannot be given new text or a new position while the visual
        /// tree it belongs to is being rendered.
        /// </summary>
        private void RefreshAxis()
        {
            var canvas = contentRect;
            if (canvas.width > 4f && canvas.height > 4f)
            {
                surface.SetView(canvas, navigator.View, Margin, preserveAspect: false);
                SyncAxisLabels(navigator.View);
            }
        ...
        /// <summary>The distance between ticks on each axis: a 1-2-5 step no finer than the pixels can tell apart.</summary>
        private Vector2 TickSteps() => new(
            AxisScale.NiceStep(48f / Mathf.Max(surface.Scale.x, 1e-6f)),
            AxisScale.NiceStep(32f / Mathf.Max(surface.Scale.y, 1e-6f)));
        ...
        private void PaintGrid(Painter2D painter, Rect boundsRect)
        {
            ...
            for (var x = Mathf.Ceil(boundsRect.xMin / step.x) * step.x;
                 x <= boundsRect.xMax + 1e-4f;
                 x += step.x)
                StrokeDataLine(painter, new Vector2(x, boundsRect.yMin), new Vector2(x, boundsRect.yMax));
            for (var y = Mathf.Ceil(boundsRect.yMin / step.y) * step.y;
                 y <= boundsRect.yMax + 1e-4f;
                 y += step.y)
                StrokeDataLine(painter, new Vector2(boundsRect.xMin, y), new Vector2(boundsRect.xMax, y));
            ...
            StrokeDataLine(painter, new Vector2(boundsRect.xMin, 0f), new Vector2(boundsRect.xMax, 0f));
            StrokeDataLine(painter, new Vector2(boundsRect.xMin, 1f), new Vector2(boundsRect.xMax, 1f));
        }
        ...
        /// <summary>
        /// Writes one label per grid line, at the step the grid is drawn on, so the numbers always name the
        /// lines under them however far the view is zoomed or panned.
        /// </summary>
        private void SyncAxisLabels(Rect boundsRect)
        {
            ...
            for (var x = Mathf.Ceil(boundsRect.xMin / step.x) * step.x;
                 x <= boundsRect.xMax + 1e-4f && needed < most;
                 x += step.x)
                PlaceLabel(ref needed, AxisScale.Format(x, step.x),
                    new Vector2(x, boundsRect.yMin), new Vector2(-8f, -12f));

            for (var y = Mathf.Ceil(boundsRect.yMin / step.y) * step.y;
                 y <= boundsRect.yMax + 1e-4f && needed < most * 2;
                 y += step.y)
                PlaceLabel(ref needed, AxisScale.Format(y, step.y),
                    new Vector2(boundsRect.xMin, y), new Vector2(2f, -6f));
