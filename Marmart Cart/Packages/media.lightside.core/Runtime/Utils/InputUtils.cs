...
{
    /// <summary>
    /// Backend-agnostic input access. Calling <see cref="UnityEngine.Input"/> directly — or anything that
    /// forwards to it, such as uGUI's stock <see cref="BaseInput"/> — throws an
    ...
    public static class InputUtils
    {
        ...
        /// <summary>
        /// Whether the editor is turning mouse / pen input into touch — the Device Simulator view, or the
        /// Input System's Simulate Touch Input From Mouse or Pen. A touchscreen on the development machine
        /// does not count. Always <see langword="false"/> in players.
        /// </summary>
        public static bool EditorTouchSimulationActive
        {
            get
            {
#if !UNITY_EDITOR
                return false;
#else
                if (UnityEngine.Device.Application.isMobilePlatform) return true;
#if ENABLE_INPUT_SYSTEM
                var simulation = UnityEngine.InputSystem.EnhancedTouch.TouchSimulation.instance;
                return simulation != null && simulation.enabled;
#else
                return false;
#endif
#endif
        ...
        /// <summary>The IME composition in progress, or an empty string when none is.</summary>
        ...
        /// <summary>
        /// Turns the operating system's IME composition on or off, preferring the active EventSystem's
        /// <see cref="BaseInput"/> — under Input System-only handling that means a custom override, the
        /// only route that engages desktop Linux's shift- and layout-resolved character translation,
        /// which the Input System's own keyboard command does not.
        /// </summary>
        ...
        /// <summary>Places the IME candidate window, in the screen pixels <see cref="Input.compositionCursorPos"/> takes.</summary>
        ...
        /// <summary>
        /// The active input module's <see cref="BaseInput"/>, or <see langword="null"/> when the only one
        /// available is uGUI's stock <see cref="BaseInput"/> and the legacy input manager is off: that type
        /// forwards every member to <see cref="Input"/>, which throws under Input System-only handling.
        /// </summary>
        private static BaseInput EventSystemInput
        {
            get
            {
                ...
                var input = module != null ? module.input : null;
#if !ENABLE_LEGACY_INPUT_MANAGER
                if (input != null && input.GetType() == typeof(BaseInput)) return null;
#endif
                return input;
            }
        }
        ...
        private static string composition = string.Empty;
        private static Keyboard subscribedKeyboard;

        private static void EnsureKeyboard()
        {
            var kb = Keyboard.current;
            if (ReferenceEquals(kb, subscribedKeyboard)) return;
            if (subscribedKeyboard != null)
            {
                subscribedKeyboard.onTextInput -= OnTextInput;
                subscribedKeyboard.onIMECompositionChange -= OnCompositionChange;
            }
            subscribedKeyboard = kb;
            composition = string.Empty;
            if (kb == null) return;
            kb.onTextInput += OnTextInput;
            kb.onIMECompositionChange += OnCompositionChange;
        }
        ...
        private static void OnCompositionChange(UnityEngine.InputSystem.LowLevel.IMECompositionString value)
            => composition = value.Count == 0 ? string.Empty : value.ToString();
