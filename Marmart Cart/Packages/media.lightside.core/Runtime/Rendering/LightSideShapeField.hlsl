...
// The outlines that stream their vertices from the atlas, over the analytic ones. Kept apart from those so a
// caller carrying only an analytic kind — a pattern's element, which its provider validates — cannot reach a
// row index through parameters that mean something else entirely.
float evalShapePrimitiveSdf(float kind, float2 p, float2 b, float4 prm, float aux, float style)
{
    float carve = (style > 0.5) ? 0.0 : aux;
    float result;
    if (kind < 17.5)      result = evalShapeAnalyticSdf(kind, p, b, prm, aux, style);
...
// The unit axis a pattern's curve is measured along, as its paint frame was turned. Its own texel, so a
// drive that does not turn never reads it.
float2 evalShapePatternAxis(float rowV)
{
    return LIGHTSIDE_SAMPLE_SHAPE_VERTS(4.5 / UI_SDF_VERT_TEX_WIDTH, rowV).zw;
}

// Coverage of one repeating pattern cell at `coord` (measured in frame units), from the description packed
// into row `rowV` of the shape-vertex atlas by ShapePattern.Pack: the element's own outline, the lattice it
// repeats on, the turn and size each cell gives it, and a curve that sizes an element by where it stands.
// The element is evaluated through the same primitive library the shape itself uses, so it is exact and
// antialiased across one pixel whatever the cell count; softness spreads that edge, it does not create it.
float evalShapePatternCoverage(float2 coord, float rowV, float driveT)
{
    float4 t0 = LIGHTSIDE_SAMPLE_SHAPE_VERTS(0.5 / UI_SDF_VERT_TEX_WIDTH, rowV);
    float4 t1 = LIGHTSIDE_SAMPLE_SHAPE_VERTS(1.5 / UI_SDF_VERT_TEX_WIDTH, rowV);
    float4 t2 = LIGHTSIDE_SAMPLE_SHAPE_VERTS(2.5 / UI_SDF_VERT_TEX_WIDTH, rowV);
    ...
    // The curve is baked as four samples per texel over the drive's domain, which the caller chose, measured
    // and folded; a drive of None baked it flat, so what arrives there does not matter.
    ...
    float size = max(lerp(a, b, u - lo), 1e-4);

    // An alternating lattice leaves every other cell empty; reporting a distance no falloff reaches is how a
    // cell says it carries nothing.
    if (t3.w > 1.5 && frac((id.x + id.y) * 0.5) > 0.25) return 0.0;

    float2 turned = float2(t3.x * local.x + t3.y * local.y, -t3.y * local.x + t3.x * local.y);
    float d = evalShapeAnalyticSdf(t0.x, turned / size - t2.zw, t2.xy, t1, t0.y, t0.z) * size;
    float aa = max(max(fwidth(lattice.x), fwidth(lattice.y)), 1e-6) * 0.5 + t3.z;
...
float evalCompositeSdf(float2 p, float2 b, float rowV, int count, float2 b0)
{
    ...
    float d = 1e6;
    [loop] for (int i = 0; i < UI_SDF_MAX_COMPOSITE_ELEMENTS; i++)
    {
        if (i >= count) break;
...
{
    float result;
    if (kind < 19.5) result = evalShapePrimitiveSdf(kind, p, b, prm, aux, style);
    else             result = evalCompositeSdf(p, b, LightSideShapeRowV(prm.x), (int)(prm.y + 0.5), prm.zw);
    return result;
}
