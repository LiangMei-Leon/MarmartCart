...
struct pixel_t
{
    ...
    float4 shapeGeom   : TEXCOORD9;  // halfSize.xy, aux, trailing mode param — glyphs: .z is the deform lane (UV1.x)
    float4 shapeParams : TEXCOORD10; // per-shape params (radii / ratios / counts)
#else
    float4 shapeGeom   : TEXCOORD6;  // halfSize.xy, aux, trailing mode param — glyphs: .z is the deform lane (UV1.x)
...
#endif
{
    ...
    {
        half4 c = UNITY_SAMPLE_TEX2DARRAY(_LightSideLottieAtlas, atlasUV.xyz) * input.color;
