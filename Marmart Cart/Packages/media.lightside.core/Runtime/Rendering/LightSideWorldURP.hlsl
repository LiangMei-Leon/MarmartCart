...
static float2 lightSideShapeUV;
#define LIGHTSIDE_PAINT_DISTANCE_T lightSideDistanceT
#define LIGHTSIDE_PAINT_SHAPE_UV lightSideShapeUV
...
// The shape surface comes first: a pattern paint repeats an outline through the primitive library
// that header brings, so the macro below must resolve to a function that already exists.
#define LIGHTSIDE_PAINT_PATTERN(coord, row, driveT) evalShapePatternCoverage(coord, LightSideShapeRowV(row), driveT)
#define LIGHTSIDE_PAINT_PATTERN_AXIS(row) evalShapePatternAxis(LightSideShapeRowV(row))
...
#endif
    float4 shapeGeom   : TEXCOORD10; // halfSize.xy, aux, trailing mode param — glyphs: .z is the deform lane (UV1.x)
...
{
    // Deformation displaces the sample point ahead of the derivatives, so the AA ramp widens exactly
    // where the field was stretched; a no-op on every quad no deformation touched. The HDRP graph
    // wrappers pack no shape geometry, so the deform lane reads 0 there and text renders undeformed.
    ...
    if (isShape)
    {
        ...
        lightSideShapeUV = input.glyphUV / max(input.shapeGeom.xy, 1e-4);
    }
#endif
    ...
    else if (atlasUV.w > 3.5)
    {
        half4 c = SAMPLE_TEXTURE2D_ARRAY(_LightSideLottieAtlas, sampler_LightSideLottieAtlas,
            atlasUV.xy, atlasUV.z) * input.color;
        result = half4(c.rgb * c.a, c.a);
    }
    else if (atlasUV.w > 1.5)
    {
        half4 color = LIGHTSIDE_SAMPLE_COLOR_TEXEL_GRAD(atlasUV.xyz, uvDx, uvDy);
    ...
    else
    {
        ...
        float coverage = LightSideResolveGlyphCoverage(atlasUV, input.cov, input.extra, aa, uvDx, uvDy);
