...
namespace LightSide
{
    public abstract partial class GpuAtlas<TEntry> where TEntry : struct, IGpuAtlasEntry
    {
        ...
        {
            GL.Begin(GL.QUADS);
            EmitBlitQuad(dstX0, dstY0, dstX1, dstY1, srcU0, srcV0, srcU1, srcV1);
            GL.End();
        }

        /// <summary>One quad of an open <see cref="GL.QUADS"/> batch.</summary>
        private static void EmitBlitQuad(float dstX0, float dstY0, float dstX1, float dstY1,
            float srcU0, float srcV0, float srcU1, float srcV1)
        {
        ...
        /// <summary>Moves each live tile old→new at mip 0 via the blit primitive — one draw per (destination page, source page) pair, so a compaction costs pages², never tiles, in draw calls. Mip-less atlases only — mip-window (pixel-tile) atlases do not compact.</summary>
        private void BlitCompactedTiles(RenderTexture source, RenderTexture destination)
        {
            ...
            compactList.Sort(compactPageOrder);
            var previousActive = RenderTexture.active;
            bool previousSrgb = GL.sRGBWrite;
            GL.sRGBWrite = !isLinear;
            try
            {
                ...
                int i = 0;
                while (i < compactList.Count)
                {
                    int dstPage = compactList[i].newPage;
                    int srcPage = compactList[i].oldPage;
                    Graphics.SetRenderTarget(destination, 0, CubemapFace.Unknown, dstPage);
                    material.SetFloat(GpuAtlasShared.BlitSliceId, srcPage);
                    material.SetPass(0);
                    GL.PushMatrix();
                    GL.LoadOrtho();
                    GL.Begin(GL.QUADS);
                    for (; i < compactList.Count; i++)
                    {
                        var rec = compactList[i];
                        if (rec.newPage != dstPage || rec.oldPage != srcPage) break;
                        ...
                        EmitBlitQuad(dstX * invPage, dstY * invPage,
        ...
        private static readonly Comparison<CompactRecord> compactPageOrder = static (a, b) =>
        {
            int byDestination = a.newPage.CompareTo(b.newPage);
            return byDestination != 0 ? byDestination : a.oldPage.CompareTo(b.oldPage);
        };
