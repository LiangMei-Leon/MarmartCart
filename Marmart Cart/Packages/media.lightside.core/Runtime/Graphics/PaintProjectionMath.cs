    ...
    public static class PaintProjectionMath
    {
        ...
        public static PaintFrame BuildFrame(in PaintProjection projection, Rect bounds)
        {
            switch (projection.kind)
            {
                ...
                case PaintProjectionKind.Pattern:
                    break;
                default:
        ...
        /// source that every kind but <see cref="PaintProjectionKind.Linear"/> samples: anything but
        /// <see cref="PaintFit.Stretch"/> equalizes the axes, so a ring stays circular, a sweep keeps true
        /// angles, and a pattern keeps square cells on a non-square frame. <see cref="PaintProjectionKind.Linear"/>
