...
{
    /// <summary>
    /// How a sampled paint progresses through its projection frame. Every member's shader word is its value
    /// plus one (<see cref="LightSidePaintKind.OfProjection"/>), so the values 3 to 5 stand reserved for the
    /// texture, tiled-texture and distance words that other paths emit; a new member takes the next value
    /// above them.
    /// </summary>
    public enum PaintProjectionKind : sbyte
    {
        ...
        /// <summary>
        /// A repeating outline, described by the consumer's own pattern: the parameter is the element's
        /// antialiased coverage, so the ramp colours what the pattern covers. A consumer that supplies no
        /// pattern reads this kind as its linear projection.
        /// </summary>
        Pattern = 6,
    }
    ...
    /// <summary>
    /// How a sampled source continues outside its projection frame. A
    /// <see cref="PaintProjectionKind.Pattern"/> covers every frame it reaches, so there it continues the
    /// curve its elements are sized by instead: repeating or mirroring the falloff outward rather than
    /// holding it at the frame's edge.
    /// </summary>
    ...
    public static class PaintSpreadExtensions
    {
        ...
        /// float. A power of two above every paint-kind word, so a shader recovers both parts exactly at
        /// any interpolator precision.
        /// </summary>
        public const float CodeStep = 16f;
        ...
        /// <summary>
        /// Packs a spread mode, and what a pattern reads and where its description sits, onto
        /// <paramref name="kindCode"/> for a vertex paint payload. Each rides its own power-of-two lane — kind
        /// below 16, spread to 64, the pattern's drive to 256, its atlas row above — so a shader recovers every
        /// part exactly from the one float, and a paint carrying no pattern packs to what it always did.
        /// </summary>
        public static float Pack(this PaintSpread spread, float kindCode, int patternRow = 0,
            int patternDrive = 0)
            => kindCode + CodeStep * (float)spread
               + 64f * Mathf.Clamp(patternDrive, 0, 3) + 256f * Mathf.Max(patternRow, 0);
    }
