...
namespace LightSide
{
    ...
    /// <summary>
    /// A timeline source that turns objects dropped on a lane into a clip. The window asks before showing a
    /// drop cursor and calls <see cref="DropClip"/> once the drop lands.
    /// </summary>
    public interface ITimelineDropTarget
    {
        /// <summary>Whether a clip can be made from <paramref name="objects"/>.</summary>
        bool AcceptsDrop(IReadOnlyList<Object> objects);

        /// <summary>
        /// Creates a clip at <paramref name="start"/> on <paramref name="track"/> bound to <paramref name="objects"/>
        /// and returns its identity; <paramref name="anchor"/> is where, in the window's panel, any follow-up
        /// choice should open.
        /// </summary>
        object DropClip(int track, float start, IReadOnlyList<Object> objects, Rect anchor);
    }
