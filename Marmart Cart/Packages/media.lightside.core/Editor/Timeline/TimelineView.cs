...
namespace LightSide
{
    ...
    public sealed class TimelineView : VisualElement
    {
        ...
        private void AppendTrackRow()
        {
            ...
            lane.RegisterCallback<DragUpdatedEvent>(evt =>
            {
                if (source is not ITimelineDropTarget target ||
                    !target.AcceptsDrop(DragAndDrop.objectReferences))
                    return;
                DragAndDrop.visualMode = DragAndDropVisualMode.Copy;
                evt.StopPropagation();
            });
            lane.RegisterCallback<DragPerformEvent>(evt =>
            {
                var objects = DragAndDrop.objectReferences;
                if (source is not ITimelineDropTarget target || !target.AcceptsDrop(objects)) return;
                DragAndDrop.AcceptDrag();
                var track = trackRows.IndexOf(row);
                var time = Snap(TimeAt(lane.WorldToLocal(evt.mousePosition).x), false, out _);
                var id = target.DropClip(track, time, objects, new Rect(evt.mousePosition, Vector2.one));
                Refresh();
                if (id != null)
                {
                    selected.Clear();
