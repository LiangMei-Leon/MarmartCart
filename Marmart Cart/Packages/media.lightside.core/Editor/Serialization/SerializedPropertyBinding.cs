...
namespace LightSide
{
    ...
    public sealed class SerializedPropertyBinding
    {
        ...
        /// <summary>
        /// Edits this serialized property independently on every selected target as one discrete undo
        /// step that restores each target whole — the form every structural edit takes, so an element
        /// with a managed reference comes back from Undo with all of its data on a prefab instance too.
        /// </summary>
        ...
        /// <summary>
        /// A discrete edit opens its own undo group and records each target whole before the change:
        /// Unity's property-level undo re-creates a removed managed reference from the modifications it
        /// recorded, which on a prefab instance leaves every other field of the restored object at its
        /// zero value; the whole-object record restores it intact.
        /// </summary>
        private void EditTargets(string undoName, bool discrete,
            Action<Object, SerializedProperty> edit)
        {
            if (discrete) Undo.IncrementCurrentGroup();
            ...
            foreach (var target in targets)
            {
                ...
                if (discrete) Undo.RegisterCompleteObjectUndo(target, undoName);
                var targetState = new SerializedObject(target);
                targetState.UpdateIfRequiredOrScript();
                var property = targetState.FindProperty(propertyPath) ??
                               throw MissingProperty(target);
