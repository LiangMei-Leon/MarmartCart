...
namespace LightSide
{
    ...
    public abstract class InspectorPopupWindow : EditorWindow
    {
        ...
        protected internal VisualElement PreparePopup(params string[] blockClasses)
        {
            ...
            // A popup panel is a form of its own: without the axis its labelled fields find no container to
            // measure against and each keeps its own label width, so the value column steps between rows.
            panel.AddToClassList(InspectorVisuals.FormAxisClass);
