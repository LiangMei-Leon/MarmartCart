...
namespace LightSide
{
    ...
    public readonly struct OptInField
    {
        ...
        /// <summary>Optional explanation for the row and menu; the bound field's <see cref="TooltipAttribute"/> stands in when omitted.</summary>
    ...
    public static class OptInParameters
    {
        ...
        public static VisualElement CreateList(SerializedProperty owner,
            ...
            => CreateList(owner, fields, label, out _);

        /// <summary>
        /// Creates a list over an explicit schema and hands back its <paramref name="header"/>, for a host that
        /// carries its own controls up there — the rows a value always shows, beside the add action that reveals
        /// the ones it rests without.
        /// </summary>
        public static VisualElement CreateList(SerializedProperty owner,
            IReadOnlyList<OptInField> fields, string label, out InspectorListHeader header)
        {
            header = null;
            ...
            var list = new OptInParameterList(owner, schema, label);
            header = list.Header;
            return list;
        }
        ...
        private sealed class OptInParameterList : VisualElement
        {
            ...
            /// <summary>The list's own header, for a host that carries controls beside the add action.</summary>
            internal InspectorListHeader Header => list.Header;
            ...
            public OptInParameterList(SerializedProperty owner, OptInField[] schema, string label)
            {
                ...
                tooltips = new string[schema.Length];
                ...
                for (var i = 0; i < schema.Length; i++)
                {
                    ...
                    tooltips[i] = schema[i].Tooltip ?? SerializedPropertyField.Tooltip(memberBindings[i]);
                    observed[i + 1] = member;
                }
