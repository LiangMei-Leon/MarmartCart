...
namespace LightSide
{
    ...
    {
        /// <summary>USS class a value hosted in a disclosure's input column wears so it fills that column.</summary>
        public const string HeaderValueClass = "lightside-paint-swatch__value";
        ...
        public static object Structure(SerializedProperty swatch, SerializedPropertyBinding binding)
        {
            ...
            return (
                ...
                kind == PaintSourceKind.Gradient && ShowsPattern(mixedShape, shape));
        }
            ...
            SerializedPropertyBinding binding, Action<VisualElement, SerializedProperty> extraRows = null)
        {
            ...
            if (!mixedKind)
            {
                ...
                valueField.AddToClassList(HeaderValueClass);
        ...
        /// <summary>Whether a gradient's spread row applies — every projection but the sweep, whose own parameter already spans exactly one period.</summary>
        ...
        /// <summary>Whether the pattern's own description applies — only <see cref="PaintProjectionKind.Pattern"/> reads it.</summary>
        public static bool ShowsPattern(bool mixedShape, PaintProjectionKind shape)
            => mixedShape || shape == PaintProjectionKind.Pattern;
