...
namespace LightSide
{
    ...
    public interface IEditSurface
    {
        ...
        /// <summary>Strokes a circle of <paramref name="radiusPixels"/> screen pixels about a working-plane point, <paramref name="widthPixels"/> pixels wide.</summary>
        void DrawCircle(Vector2 plane, float radiusPixels, float widthPixels, Color color);

        /// <summary>
        /// Working-plane units one screen pixel spans at <paramref name="plane"/> — what turns a pixel offset
        /// into the plane distance that holds it at any zoom or camera distance.
        /// </summary>
        float PlaneUnitsPerPixel(Vector2 plane);
    }
}
